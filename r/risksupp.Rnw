...
message=FALSE, autodep=TRUE, fig.path='figure/', fig.width=6, par=TRUE)
...
\title{Supplementary material on the modelling process}
\author{
Author list
}
\date{}
...
First, we'll load the data and subset the data for the Neogene period:
...
fit_models <- function(dat, test_fraction = 0.5, threshold = 0.5,
  ...
  interaction.depth = 1, n.trees = 200, bin = 0.025) {
...
val_test <- rdply(50, fit_models(stcen, shrinkage = 0.1,
    interaction.depth = 1, n.trees = 200)$bin_validation)
@
...
val_test_int <- rdply(50, fit_models(stcen, shrinkage = 0.1,
    interaction.depth = 5, n.trees = 200)$bin_validation)
val_test_ntrees <- rdply(50, fit_models(stcen, shrinkage = 0.1,
    interaction.depth = 1, n.trees = 500)$bin_validation)
@
...
% Switch the load chunk eval to FALSE and the next chunk eval to TRUE
% to re-run the 500 rep version.
<<gbm-no-mal-load, eval=TRUE, echo=FALSE>>=
load("stcen2.rda")
load("val_test2.rda")
load("val_test2_summarized.rda")
@
...
save(stcen2, file = "stcen2.rda")
save(val_test2, file = "val_test2.rda")
save(val_test2_summarized, file = "val_test2_summarized.rda")
@
