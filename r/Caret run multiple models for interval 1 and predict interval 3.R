...
data <- drop.levels(subset(data, data$use==1 & data$eac > 0))
head(data)
...
data1 <- drop.levels(subset(data,data$Interval_Name == "Middle Miocene" & data$occurrences > 1))
...
data2 <- drop.levels(subset(data,data$Interval_Name == "Upper Miocene" & data$occurrences > 1))
...
lat.range <- Quartiles(max.lat-min.lat)
...
data2factors.gen <- drop.levels(na.omit(data.frame(class,group,genus,richness,occupancy,min.lat,max.lat,lat.range,Ex)))  
### remove groups that do not occur in both intervals
...
### designate categorical response variable
ext2 <- as.factor(ifelse(data2factors$Ex == 1,"extinct","survive"))
...
ext2Pred <- extractProb(list(gbm = gbmFit2))
str(ext2Pred)
prediction <- ext2Pred$extinct


All.Preds <- predict(gbmFit1,newdata = data2factors, type = "prob")
predicted <- All.Preds$extinct

plot(prediction,predicted)
cor(log(prediction),log(predicted),method="spearman")
