...
data.finite <- composite.occ2

# important: sort first by longitude:
data.finite <- data.finite[order(data.finite$species, data.finite$longitude), ]
d <- data.finite

# clean up for memory:
rm(data.finite, composite.occ2)
gc()
...
d <- d[!duplicated(d[,c("group", "match", "genus", "long.g", "lat.g")]),]

# now fill in cells based on bounding boxes:
# X is the maximum set of longitude degrees to fill

fill.bounding.boxes <- function(long.g, lat.g, X = 90, group = NA, this.match = NA, genus = NA) {
           n.row <- length(long.g)
           long.to.fill <- c()
...
# and build the output data.frame:
  ...
  out.df$group = group
  out.df$match = this.match
  out.df
}
           
require(data.table) # super fast split apply combine
dtb <- data.table(d)
out <- dtb[, fill.bounding.boxes(long.g = long.g, lat.g = lat.g, group = group[1], this.match = match[1]), by=list(genus)]
rm(dtb)
gc()
composite.occ2.filled <- as.data.frame(out)
rm(out)
gc()

save(composite.occ2.filled, file = "../data/composite.occ2.filled.rda")
