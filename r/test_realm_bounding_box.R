...
for(current_genus in genuses_sample) {
  inter_occurs.df <- data.frame(occurs = TRUE,
    PROV_CODE = subset(d, genus == current_genus)$PROV_CODE)
  inter_does_not_occur.df <- data.frame(occurs = FALSE,
    PROV_CODE = unique(d$PROV_CODE)[!unique(d$PROV_CODE) %in% inter_occurs.df$PROV_CODE])
  inter_occur.df <- rbind(inter_occurs.df, inter_does_not_occur.df)
  inter_occur.df$occur_type <- "interpolated"

  raw_occurs.df <- data.frame(occurs = TRUE,
    PROV_CODE = subset(oc.df, genus == current_genus)$PROV_CODE)
  raw_occurs.df <- raw_occurs.df[!duplicated(raw_occurs.df), ]
  raw_does_not_occur.df <- data.frame(occurs = FALSE,
    PROV_CODE = unique(d$PROV_CODE)[!unique(d$PROV_CODE) %in% raw_occurs.df$PROV_CODE])
  raw_occur.df <- rbind(raw_occurs.df, raw_does_not_occur.df)
  raw_occur.df$occur_type <- "raw"

  with(er.df, plot(long, lat, asp = 1,type = "n", xlim = c(-180, 180),
      ylim = c(-90, 90), xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab = ""))
  d_ply(er.df, "group", transform, polygon(long, lat,
      col = paste(rep(brewer.pal(8, "Set1"), 99)[RLM_CODE], "30", sep = ""),
      border = NA))
  box(col = "grey50")
  ...
  # raw points
  with(subset(oc.df, genus == current_genus),
    points(longitude, latitude, pch = 4, col = "red", cex = 1))
  mtext(current_genus)
}
