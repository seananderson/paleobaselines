...
richness <- scale(round(log(data1$richness),0))
occupancy <- scale(round(log(data1$eac),0))
occurrences <- scale(round(log(data1$occurrences),0))
great.circle <- scale(round(log(data1$gcd_corrected),0))

lats <- data.frame(abs(data1$MinLat),abs(data1$MaxLat))

#paleolats & longs rounded
min.lat <- scale(ceiling(apply(lats,1,min)/Min.Lat.Bin)*Min.Lat.Bin)
max.lat <- scale(ceiling(apply(lats,1,max)/Max.Lat.Bin)*Max.Lat.Bin)
lat.range <- scale(max.lat-min.lat)
#lat.range <- ceiling((data1$MaxLat-data1$MinLat +.1)/Lat.Range.Bin)*Lat.Range.Bin
mean.lat <- scale(round(abs(data1$mean_lat),0))
