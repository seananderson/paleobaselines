...
PaleoDB  <- drop.levels(subset(PaleoDB,PaleoDB$latitude > -74.81682 &  PaleoDB$latitude < 81.75957))
mydata <- drop.levels(subset(mydata,mydata$paleolatdec > -74.81682 &  mydata$paleolatdec < 81.75957))
...
write.table(PaleoDB_grid_cells,"PaleoDB_06.11.13_grid_cells.csv",sep=",")
save(PaleoDB_grid_cells,file="~/Dropbox/nescent_extinction_map/Final data/PaleoDB_06.11.13_grid_cells.rda")

L <- length(PaleoDB_grid_cells[,1])

PB <- data.frame(seq(1,L,by=1),matchLong,matchLat)
colnames(PB) <- c("EID","X","Y")
  
PB <- as.EventData(PB)
cells <- makeGrid(x = global_45x14$longitude, y =global_45x14$latitude)
grid.PaleoDB.data.2 <- function(){
fc <- findPolys(PB, cells, maxRows = 1e+07)
fc_ps <- fc[,c("PID", "SID")]
...
byRecord <- ddply(PaleoDB,.(occurrence),grid.PaleoDB.data.2, .progress="text")