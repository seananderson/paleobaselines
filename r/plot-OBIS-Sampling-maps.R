...
##rm(list = ls())
source("~/Dropbox/nescent_extinction_map/r/Spalding.Map.FINAL.R")
...
id <- "mean-log"
PDF <- FALSE
plot_order <- data.frame(class = c("Mammalia", "Testudines", "Echinoidea", "Decapoda", "Anthozoa", "Gastropoda", "Bivalvia", "Elasmobranchii"), plot_order = 1:8)
# y-axis labels:
...
if(plot_type == "ext") {
  ...
  er.df <- transform(er.df, value.to.plot = (ext.plot)) # already logged
}
...
# And make the maps:
filename <- paste("class-risk-maps", today, "iter", iterations, id, type, plot_type, sep = "-")
#filename <- "temp"
if(PDF) {
  pdf(paste0(filename, ".pdf"), width = 6, height = 6.05)
} else {
  png(paste0(filename, ".png"), width = 6, height = 6.05, units = "in", res = 250)
}
