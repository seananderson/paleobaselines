...
pdf("../figs/partial-plot-base-cull-0.75.pdf", width = 5.9, height = 5.0)
layout(l)
...
plyr::d_ply(o, "predictor", function(x) {
  ...
  if(ii == 3 & length(unique(o$stage)) > 1) {
    ...
    legend(-2.11, par("usr")[4] * 1.45, legend = legend_stages, bty = "n",
...
# TODO FIX THIS!!!!!!!!! 20140924
if("Lower Miocene" %in% names(g)) {
...
mtext("Marginal effect on probability of extinction", side = 2,
