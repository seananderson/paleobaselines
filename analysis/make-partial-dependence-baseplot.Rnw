...
o.names <- data.frame(predictor = c("occupancy", "occurrences", "richness",
  ...
  predictor.clean = c("Occupancy", "Occurrences", "Richness",
    "Great circle distance", "Maximum latitude", "Minimum latitude", "Mean latitude",
    "Latitude range"), axis.1 = c(rep("c(0, 0.5)", 3), "c(5000, 15000)",
    rep("c(0, 30, 60)", 3), "c(0, 30, 60)"),
...
plyr::d_ply(o, "predictor", function(x) {
  ii <<- ii + 1
x <- x[order(x$value), ]
plot(1, 1, xlim = eval(parse(text = unique(x$xlim))), ylim = c(o.ylim.l, o.ylim.u), ann = FALSE,
