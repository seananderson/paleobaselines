...
\title{Supplementary Materials for Paleontological Baselines for Evaluating Contemporary Extinction Risk in Coastal Oceans}
...
You can install the package by downloading the \texttt{paleorisk} folder from \url{https://github.com/seananderson/riskmaps} and running:
...
Some of the code in this PDF document has been suppressed for brevity, however, the complete analysis is available by downloading the \texttt{.Rnw} file.
...
\subsection{Standardizing the predictor variables}
...
Sampling of species' distributions in the contemporary OBIS data set is incomplete.
Therefore, we used an interpolation algorithm to infer the contemporary distribution of genera.
We assumed that if a genus was identified within a biogeographic realm \cite{spalding2007}, it was likely to be found throughout that realm.
We used a minimum-bounding-box method to interpolate within biogeographic realms across biogeographic provinces.
We are assuming that the interpolated contemporary biogeographic provinces cover the ``true'' range.
This minimum-bounding-box method is similar to that used in Ref.~\cite{belanger2012}.

TODO expand this last sentence slightly so that people don't miss the point that we aren't the first to attempt to account for variable sampling in this way

We didn't interpolate the marine mammals or elasmobranch datasets since these were based on range maps.
...
As another example on a larger scale, the following plot shows the interpolations for 6 genera. The red crosses indicate sampling observations in the OBIS database. The various biogeographic realms are shown in different colour shadings.
The outlined provinces are all the provinces within any realm that has occurrences.
The grey-shaded polygons represent biogeographic provinces that are added based on the minimum-bounding-box interpolation scheme.
Outlined provinces that aren't shaded are provinces that the bounding box didn't overlap.
Keep in mind that some realms wrap from one side of the plot to the other.
...
For the paleontological dataset, we overlaid the same grid, took the centre points of the grid cells for cells that overlapped any occurrences, and calculated the range statistics on the centre points from the grid overlay.

We have suppressed the interpolation code for brevity.
<<interporlation, eval=FALSE, echo=TRUE>>=
source("../r/README_interpolation.R")
@

%The interpolation process can be re-run by sourcing the file \texttt{README\_interpolation.R} in the \texttt{r} folder. Note that this script will take a very long time to run and requires considerable RAM.


\textit{Within each realm, we bounded the outermost points with a rectangle and recorded all provinces that the bounding box crossed.}

\textit{We then overlaid a 45x14 cell equal-area grid (longitudes starting at -180 degrees and increasing by 8 degrees, latitudes at -74.8, -55.6, -43.3, -33.1, -24, -15.5, -7.3, 0.7, 8.7, 17, 25.5, 34.8, 45.3, 58.2, 81.8) and recorded the centre coordinates for all grid cells that overlapped with a province with observations or interpolated observations.}

\section{Modelling contemporary extinction risk with paleontological data}

\subsection{Choice of gradient boosting models}

Why ensemble learning method in general

TODO - note that this section is merely a grouping of quotes and notes right now. Not cohesive yet.

%[Some draft notes below. See the nescent_extinction_map/literature folder for PDFs and citation data.  - SA]
- good overall and highly cited introduction to boosted regression for ecology: \cite{elith2008}

Advantages of gbms over random forests: (From Elith et al. 2008)
``While BT (bagged trees) and RF (random forests) reduce variance more than single trees, they cannot achieve any bias reduction, because each tree is based on a bootstrap sample that will be distributed in much the same way as the original training set. As a consequence, the resulting average bias is identical to the bias of any one tree. By contrast, boosting grows the suite of trees by sequentially modelling the residuals throughout all parts of the data space, including those for atypical observations that depart from the dominant patterns explained by the initial trees. In this way, it reduces both bias (through forward stagewise fitting) and variance (through model averaging).''

We used GBMs (\cite{friedman2001}; ``gradient boosting machine'' models or ``generalized boosted regression models'') to predict \ldots GBMs are a form of supervised machine learning based on an ensemble of decision trees. Supervised refers to the fact that the model is built with training data and ensemble of decision trees refers to how the final model is a product of many weakly informative models fit in succession. Compared to parametric modelling, machine learning makes minimal assumptions about the relationship between predictors and response and prioritizes predictive performance over probabilistic inference about the model itself.

Boosting models work, in general, by building a sequence of models, each fit to the residuals of the previous model with larger residuals being given stronger weights. This means that each subsequent model focuses more heavily on data which were poorly fit by previous models. In theory, this ensemble of weak models can be combined to produce one strong overall predictive model with reduced bias compared to other machine learning methods, such as random forests \cite{elith2008}.

Although not universal, boosted tree methods have been shown to outperform other machine learning methods in many cases \cite{caruana2006} (e.g. and Niculescu-Mizil 2006), and are commonly used in applied prediction problems (e.g. by Yahoo for website rankings; \cite{cossock2008}. Early work with our dataset demonstrated that GBMs outperformed other common machine learning techniques (in both bias and precision?), such as random forests, in out of sample prediction.

TODO explain out of sample

We fit GBMs with the \pkg{gbm} package \cite{gbm2013} for the \proglang{R} statistical language and environment \cite{r2013}. The \pkg{gbm} package implements GBMs based on Ref.\ \cite{friedman2001} with some minor modifications (gbm vignette REF). Further, the \pkg{gbm} package implements the stochastic method proposed in Ref.\ \cite{friedman2001} in which a random subsample of the data is chosen at each model iteration. This random subsample is used to fit the model before computing residuals for that step thereby reducing chances of overfitting the data. At each step, we left out 50\% of the data as a random subset --- the default in the \texttt{gbm()} function.
...
<<legacy-globals, echo=FALSE, eval=FALSE>>=
# Min_Occupancy <- 1
...
TODO describe these arguments here
...
TODO add a sentence talking about the causes of non-linear bias
...
<<plot-cross-interval-gbms, fig.cap="Cross-interval comparison of genus risk estimates from gradient boosting models. The x-axis shows the probability of extinction as predicted by a model built on data from the same stage. The y-axis shows the probability of extinction as predicted by a model built on data from a different stage. The colours show different taxonomic classes. Note the log-distributed x- and y-axes. Since there is some stochasticity involved in the gradient boosting machine fitting routine, there is some variability in predictions within the same stage (the diagonal panels). In general, there is substantial similarity in extinction regimes between succeeding intervals with respect to the evaluated taxonomic and geographic range predictor variables.", fig.width=9, fig.height=7, cache=TRUE, echo=TRUE>>=
...
We will now look at the marginal effect of each predictor on extinction risk. We can do this through partial dependence plots, as shown in Fig.~1. This procedure finds the marginal effect of each predictor by integrating across all other predictor values using the method described in Friedman \cite{friedman2001}.
...
TODO note how we centered the marginal effects to compare across intervals
...
We can re-create Fig.~1 in the paper, which is a more customized version of the previous plots, by running the code in \texttt{make-partial-dependence-plot.Rnw}. We'll suppress the code here to save space.
...
Now, we'll create Fig.~1 and Fig.~2 from the paper.
