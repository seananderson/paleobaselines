...
<<test-gbm1>>=
val_test <- rdply(25, validate_gbm(stcen, shrinkage = 0.05,
...
<<alternative-gbms>>=
val_test_base <- rdply(20, validate_gbm(stcen, shrinkage = 0.05,
    interaction.depth = 1, n.trees = 300,
    use_weights = TRUE)$bin_validation)
val_test_shr <- rdply(20, validate_gbm(stcen, shrinkage = 0.01,
    interaction.depth = 1, n.trees = 300,
    use_weights = TRUE)$bin_validation)
val_test_int <- rdply(20, validate_gbm(stcen, shrinkage = 0.05,
    interaction.depth = 4, n.trees = 300,
    use_weights = TRUE)$bin_validation)
val_test_ntrees <- rdply(20, validate_gbm(stcen, shrinkage = 0.05,
...
val_test <- rdply(50, validate_gbm(stcen2, shrinkage = 0.05,
  interaction.depth = 1, n.trees = 300,
  use_weights = TRUE)$bin_validation)
...
paleo_long <- reshape2::melt(stcen2, id.vars = c("class", "group", "genus"),
  measure.vars = c("richness", "occupancy", "occurrences",
    "min.lat", "max.lat", "lat.range", "mean.lat", "great.circle"))
...
map_class_ext(subset(er_dat_class, class != "Malacostraca"), yticks = c(0.005, 0.01, 0.02, 0.05, 0.10, 0.20),
...
<<predict-no-bounding-box>>=
# Read in raw (not interpolated) contemporary data:
temp_dat <- readRDS("../data/stand-predictors-cen-obis.rds")
modern_raw <- subset(temp_dat,
  !class %in% "Testudines" & stage == "Modern_raw")
...
m2 <- modern
m2 <- plyr::rename(m2, c("richness" = "richness_bb",
    "occupancy" = "occupancy_bb",
    "occurrences" = "occurrences_bb",
    "great.circle" = "great.circle_bb"))
m2 <- plyr::join(m2, modern_raw[,c("class", "genus",
    "richness", "occupancy", "occurrences", "great.circle")])

ggplot(modern_raw, aes(mean_observed, mean_observed_raw)) +
  geom_point(alpha = 0.2) + facet_wrap(~class)

ggplot(m2, aes(richness_bb, richness)) +
  geom_point(alpha = 0.2) + facet_wrap(~class)

ggplot(m2, aes(occurrences_bb, occurrences)) +
  geom_point(alpha = 0.2) + facet_wrap(~class)

ggplot(m2, aes(occupancy_bb, occupancy)) +
  geom_point(alpha = 0.2) + facet_wrap(~class)

ggplot(m2, aes(great.circle_bb, great.circle)) +
  geom_point(alpha = 0.2) + facet_wrap(~class)
@
...
d.eco.filled <- gdata::drop.levels(subset(d.eco.filled,
    d.eco.filled$Ranges == Input_ranges))
d.eco.filled$Ranges <- NULL

# Merge occurrence data with risk predictions:
d.eco.filled <- merge(d.eco.filled, modern_raw, by = "genus", all.x = FALSE)

# And calculate mean values by class-province or for the provinces overall:
by.prov.classes.raw <- dplyr::summarise(dplyr::group_by(na.omit(d.eco.filled),
    class, PROV_CODE),
  mean.ext = mean(log(mean_observed_raw + 0.001)),
  N.gen = length(unique(genus)))

by.prov.all.raw <- dplyr::summarise(dplyr::group_by(na.omit(d.eco.filled),
    PROV_CODE),
  mean.ext = mean(log(mean_observed_raw + 0.001)),
  N.gen = length(unique(genus)))
...
<<no-bounding-box-class-map>>=
# Make the class figure:
map_class_ext(subset(er_dat_class_raw, class != "Malacostraca"),
...
pdf("../figs/hotspots-jk.pdf", width = 7, height = 9)
par(mfrow = c(4, 2))
