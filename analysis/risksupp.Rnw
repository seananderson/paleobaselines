...
<<plot comparison of modern and plio-pleistocene ranges>>=
Plio.Modern <- neog[(neog$Interval_Name == "Modern_raw" | neog$Interval_Name == "Plio-Pleistocene"),]
Plio.Modern <- Plio.Modern[ -c(15:16) ]
Plio.Modern <- melt(Plio.Modern,id=c("Interval_Name","stage","stage_top","class","group","genus"))
...
mod <- lmodel2(value.y ~ value.x, data=df,"interval", "interval", 99)
reg <- mod$regression.results
reg <- reg[(reg$Method=="RMA"),]
names(reg) <- c("method", "intercept", "slope", "angle", "p-value")
return(reg)
}
RMA <- ddply(Plio.Modern2,.(variable),each(RMA.lines))

# make plot
pdf("../figs/compare-modern-and-Plio-Pleistocene-ranges.pdf", width = 9, height = 4.5)
p <- ggplot(Plio.Modern2,aes(value.x,value.y)) +
geom_point(alpha=0.1) +
facet_wrap(~variable,scale="free",ncol=4) +
geom_abline(data = RMA, aes(intercept = intercept, slope = slope, colour = "blue")) +
geom_abline() +
xlab("Modern") +
...
map_class_ext(er_dat_class, yticks = c(0.005, 0.01, 0.02, 0.05, 0.10, 0.20),
  min_prov_genera = 10, col_range = TRUE, exact_limits = c(0.0028,
    0.33),
  ylabel = "Intrinsic extinction probability (over ~20 Myr)")
dev.off()
@
