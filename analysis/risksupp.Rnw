...
if(!file.exists("../data/val_test.rds")) {
  val_test <- rdply(300, validate_gbm(neog, shrinkage = 0.05,
    interaction.depth = 1, n.trees = 2000, 
...
val_test_int <- rdply(15, validate_gbm(neog, shrinkage = 0.05,
    interaction.depth = 4, n.trees = 2000,
    use_weights = TRUE)$bin_validation)
...
p <- p + geom_point(data=cross_pred,
  ...
  xlab("Extinction probability (stage that model was built on)") +
  ylab("Extinction probability (stage that model was tested on)") +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  geom_text(data=Rsqs,aes(.001,.70,label=Rsq),size=3.5)
  geom_text(data=Rsqs,aes(.001,.70,label=sprintf("%.2f", Rsq)),size=3.5)
print(p)
dev.off()
...
fit_stage_models <- function(dat, stage_name) {
  ...
  gbm::gbm(Ex ~ richness + occupancy + occurrences + min.lat + max.lat +
    ...
    d, n.trees = 2000, interaction.depth = 1,
...
m_relative <- gbm(Ex ~ richness + occupancy + occurrences + min.lat +
    max.lat + lat.range + mean.lat + great.circle + group,
  data = neog, n.trees = 2000, interaction.depth = 1,
  distribution = "bernoulli", shrinkage = 0.05, weights = weights)
ri <- rdply(20, function(x) {
  m_relative <- gbm(Ex ~ richness + occupancy + occurrences + min.lat +
    max.lat + lat.range + mean.lat + great.circle + group,
  data = neog, n.trees = 2000, interaction.depth = 1,
  distribution = "bernoulli", shrinkage = 0.05, weights = weights)
  gbm::relative.influence(m_relative, scale. = TRUE,
    sort. = FALSE, n.trees = 2000)
  })
...
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
labs.nbb <- seq(0.012, 0.024, 0.004)
