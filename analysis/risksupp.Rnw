...
<<check-classification-error, child='check-classification-error-child.Rnw', results='hide', fig.height=7, fig.width=12, fig.cap='Receiver operating characteristic (ROC) curves. ROC curves show sensitivity (true positive rate) plotted against specificity (true negative rate) across all possible probability thresholds. The area under the curves (AUC) represents the probability that a randomly chosen extinct genera will be correctly ranked as more at risk than a randomly chosen genera that survives. The dashed line represents the the 50\\% line --- predictions on this line would be no better than tossing a coin. We show predictions for models built on the entire Neogene (as used in Fig.~2 and Fig.~3) and for models built on stages within the Neogene. The coloured lines represent ROC curves from the same models but split out by major taxonomic groups.'>>=
@
...
<<plot-density-risk-by-province, fig.width=17, fig.height=13, fig.cap="The distribution of intrinsic extinction risk by biogeographic province \\cite{spalding2007} (panels) and taxonomic class (colours). Note the log transformed x-axis.", echo=FALSE>>=
...
<<jackknife-map, fig.cap="Global distribution of mean predicted intrinsic extinction probability (as in Fig.~3) \\textbf{with one taxonomic class removed, or jackknifed, in each panel}. Note that the colour scale is adjusted in each panel to display the full range of predictions.", fig.width=7, fig.height=7>>=
