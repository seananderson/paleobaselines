...
We are assuming that the interpolated contemporary biogeographic provinces cover the ``true'' range. A similar minimum-bounding-box approach has been used in the literature before to account for variable sampling, for example Ref.~\cite{belanger2012}.
We did not interpolate the marine mammals or elasmobranch datasets since these were based on range maps.
