...
# no single observation genera from fossil record:
if(!file.exists("../data/val_test_sing.rds")) {
  val_test_sing <- ldply(1:300, function(i)
    validate_gbm(neog[neog$single_obs == 0, ],
      shrinkage = SHRINKAGE,
      interaction.depth = INT.DEPTH, n.trees = NTREES,
      use_weights = TRUE)$bin_validation, .parallel = TRUE)
...
# val_test_plot <- val_test_sing[sample(1:nrow(val_test_1.0), size = 1000), ]
...
# no single occurrences:
if(!file.exists("../data/main_model_sing.rds")) {
  neog_subset <- neog[neog$single_obs == 0, ]
  w <- paleorisk::get_weights(neog_subset$Ex)
  weights <- ifelse(neog_subset$Ex == 1, w$ex_weight, w$sur_weight)
...
max_range <- map_class_ext(er_dat_class, yticks = c(0.005, 0.01, 0.02, 0.05, 0.10),
  min_prov_genera = 10, col_range = TRUE, exact_limits = c(0.0034, 0.15),
  picture_files = picture_files, plot_column = "mean.ext.1.0",
  silhouette_coords = silhouette_coords,
  ylabel = "Intrinsic extinction probability (over ~23 Myr)")
dev.off()
...
labs <- c(0.012, 0.015, 0.02)#, 0.025, 0.03)
labs_show <- sprintf("%.3f", labs)
COL_LIMITS <- c(0.010, 0.03)
map_hotspots(er_dat, min_prov_genera = 100, add_legend = TRUE,
  hotspots = TRUE, at = log(labs), at.labels = labs_show)
...
map_hotspots(er_dat, min_prov_genera = 100, add_legend = TRUE,
  hotspots = TRUE, at = log(labs), at.labels = labs_show,
  #hotspots = TRUE, at = log(seq(0.15, 0.16, 0.01)), at.labels = seq(0.13, 0.16, 0.01),
  plot_column = "mean.ext.1.0")#, col_limits = COL_LIMITS)
par(xpd = NA)
mtext("Intrinsic extinction probability", side = 4, outer = TRUE,
  line = -1.4, las = 0, col = "grey30", cex = 0.9)
dev.off()
...
pdf("../figs/class-map-obis-culls.pdf", width = 6.45, height = 5.1)
for(i in seq_along(obis.culls)) {
  er_dat_class_obis_cull <- plyr::join(er_points, er@data, by = "id")
  er_dat_class_obis_cull <- plyr::join(er_dat_class_obis_cull,
    by.prov.classes.obis.culls[[i]], by = "PROV_CODE")
  ...
  map_class_ext(er_dat_class_obis_cull,
    ...
    silhouette_coords = silhouette_coords,
    min_prov_genera = 10, col_range = TRUE,
    exact_limits = c(0.0034, 0.15), max_range_given = max(max_ranges),
    ylabel = "Intrinsic extinction probability (over ~23 Myr)")
}
dev.off()
@
