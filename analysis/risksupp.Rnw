...
ggplot(d2, aes(y = jitter(gbm_pred_binned, amount = 0.02), x = jitter(great.circle, amount = 800))) +  geom_hline(yintercept=mean(d2$gbm_pred_binned),colour="red") + geom_point(alpha=.5,pch=1,size=2,colour="cadetblue") + facet_wrap(~class) + theme_bw() + xlab("Great circle distance") + ylab("Predicted intrinsic risk by genus") + theme(strip.text.x = element_text(size = 8), axis.text = element_text(size = 8)) + theme(legend.position = "none") + geom_density2d(colour="black") 
@
