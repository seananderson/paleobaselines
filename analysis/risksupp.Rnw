...
stage_models <- llply(stage_order, function(x)
  fit_stage_models(stcen2, stage_name = x))
names(stage_models) <- stage_order
...
groups_match <- data.frame(predictor_value =
...
ggplot(partial_continuous, aes(predictor_value, y)) +
...
ggplot(partial_groups, aes(y+0.001, group)) +
