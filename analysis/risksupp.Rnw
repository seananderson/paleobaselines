...
For example, here are the predicted and observed extinction probabilities for one run of our cross-validations. The model was built on half the data and then used to predict the other half of the data.
...
\caption{Consistency of relative extinction risk patterns throughout the Neogene-Pleistocene fossil record. Shown are comparisons of scaled genus extinction probabilities estimated from models built and tested on different geologic subintervals. For example, the upper right-hand panel plots the extinction risk of Plio-Pleistocene genera predicted by the model built on those data (x axis) compared to a predictions by a model built using the Lower Miocene data (y axis). The higher the rank order correlation, as shown by Spearman's Rho values in upper left panel corners (except for within-interval comparisons), the more similar the determinants of extinction risk are between two given subintervals. Note that because extinctions were weighted more heavily in the GBM models (Section~\ref{sec:build-calibrate}), the absolute values shown on the x and y axes here are transformed versions of the real probabilities. We corrected this using a calibration procedure when we applied the Neogene-Pleistocene model to the modern dataset (Section~\ref{sec:build-calibrate}). Genus risk predictions are similar across subinterval-specific models despite subinterval-to-subinterval variation in the environmental drivers of extinction and in the sampling of the fossil record.}
...
\caption{Schematic illustration of the sequence of steps used to build the modern genus range dataset and to make intrinsic risk predictions for modern genera from the Neogene-Pleistocene fossil record.}
\label{fig:didactic}
\end{figure}
...
\caption{The geographic distribution of \textbf{intrinsic risk residuals} across coastal biogeographic provinces after applying a linear model accounting for the relationship between number of OBIS records and mean intrinsic risk. Panels for sharks and mammals (as in Fig.~2) are not shown because we used range maps instead of OBIS data for the modern distributions. The broad-scale mean intrinsic risk patterns plotted in Fig.~2 remain after accounting for the positive relationship between mean intrinsic risk and number of OBIS records (Fig.~TODO-was-S15).}
...
\caption{Mean predicted intrinsic risk of genera inhabiting different ocean biogeographic provinces \cite{spalding2007} for each of the six major groups considered (upper six panels) and across all genera (lower centred panel), as in Fig.~2 and 3, but excluding \textbf{genera that occurred in only a single biogeographic province}. Scale bars on the right indicate the range of intrinsic extinction risk values for a given group of taxa. Culling these genera does not substantially alter broad-scale patterns of intrinsic risk (Fig.~2 and 3).}
...
\caption{Mean intrinsic extinction risk of genera inhabiting different ocean biogeographic provinces \cite{spalding2007} for each of the six major groups considered (upper six panels) and across all genera (lower centred panel), as in Fig.~2 and 3, but \textbf{with predictions based on raw ranges in OBIS without the within-realm minimum bounding box interpolation (Fig. TODO-was-S7)}. Scale bars on the right indicate the range of intrinsic extinction risk values for a given group. Note that the modern distribution of sharks and mammals was based on range maps instead of OBIS observations and so panels A and B are the same as in Fig.~2. Omitting the interpolation procedure results in greater heterogeneity in mean intrinsic risk among adjacent provinces but does not substantially alter broad-scale intrinsic risk patterns.}
...
\caption{Proportion genera extinct at various predictor values. TODO FILL THIS
  IN}
...
\caption{Predictors of extinction risk in the Neogene to Pleistocene marine fossil record. Partial dependence plots for the full Neogene-Pleistocene model, as in Figure 1, but varying the minimum preservation probability that fossil genera must exhibit to be included in the training data. Darker colors indicate model output when preservation thresholds are relatively lenient (e.g., all genera with preservation probabilities greater than 0), whereas lighter colours indicate model output when preservation thresholds are relatively stringent (e.g., all genera with preservation probabilities equal to 1) and poorly-preserved genera are excluded from the training data. Marginal effects of predictor variables on extinction risk are relatively similar over a wide range of preservation thresholds.}
...
\caption{Comparison of intrinsic risk estimates for modern genera from GBM models trained on different subsets of the Neogene-Pleistocene fossil record. \texttt{mean\_observed} refers to the main GBM model used in the paper. \texttt{mean\_observed\_1.0} refers to a model trained on a dataset containing only fossil genera with a preservation probability equal to one (i.e., excluding all genera that were not sampled in one or more subintervals between their first and last occurrences). \texttt{mean\_observed\_sing} refers to a model containing only fossil genera that were observed at least twice in all subintervals from the subinterval in which they first occurred through the subinterval in which they last occurred (i.e., excluding all genera known from one or fewer occurrences). Points in each panel represent an intrinsic risk prediction for an individual genus in the modern oceans based on a model trained on an specific subset of fossil data. Spearman rank-order correlation values are presented in the upper right cells. Genus instrinsic risk estimates are strongly correlated across models trained on different subsets of fossil data.}
...
\caption{Comparison of provincial mean intrinsic risk estimates from GBM models trained on different subsets of the Neogene-Pleistocene fossil record. Description of this figure is the same as in Fig. TODO-was-S21 except that points are are biogeographic province-level estimates instead of individual genus-level estimates. Spearman rank-order correlation values are presented in the upper right cells. Log mean instrinsic risk estimates for provinces are strongly correlated across models trained on different subsets of fossil data.}
...
\caption{Comparison of provincial mean intrinsic risk estimates under varying sampling thresholds applied to the OBIS database. Values along the diagonal (1, 5, 10, 20, 50) indicate the minimum number of OBIS occurrences necessary for a genus in the modern ocean to be included in the calculation of mean intrinsic risk. Points in each panel are log mean extinction risk values for coastal biogeographic provinces after applying different minimum sampling thresholds to the modern data. Spearman's rank-order correlation values are presented in the upper right cells. Provincial instrinsic risk estimates are strongly correlated across all thresholds considered.}
