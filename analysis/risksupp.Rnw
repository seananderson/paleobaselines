...
<<get-oos-rsquared, fig.width=7.5, fig.height=7, echo=TRUE>>=

# Linear regression R^2 by class:
oos_rsq_lm <- plyr::ddply(val_test, "class", function(x) {
  m <- lm(x$obs_ext_prob ~ x$gbm_pred_binned)
  summary(m)$r.squared
  })
print(oos_rsq_lm)

# Additive model deviance explained (or nearly equivalently adjusted
# R^2): (this allows for non-linear trend)
par(mfrow = c(2, 3))
oos_rsq_am <- plyr::ddply(val_test, "class", function(x) {
  require(mgcv)
  m <- mgcv::gam(obs_ext_prob ~ s(gbm_pred_binned), data = x)
  plot(m)
  mtext(unique(x$class))
  summary(m)$dev.expl
  })
print(oos_rsq_am)

# Or don't worry at all about the trend, just asked how 'off'
# predictions can be compared to the mean prediction at a given bin
# this might be what we actually care about but it's a bit more obscure
# it's basically how well those blue dots capture the variation in the
# black dots
# I used gam() here as a hack, but would switch to glm() which doesn't
# give such stats as easily
oos_rsq_factors <- plyr::ddply(val_test, "class", function(x) {
  require(mgcv)
  m <- mgcv::gam(obs_ext_prob ~ as.factor(gbm_pred_binned), data = x, weights = sample_n, family = binomial)
  summary(m)$dev.expl
  })
print(oos_rsq_factors)
...
<<no-bounding-box-overall-map, fig.cap="Mean predicted intrinsic extinction probability, as in Fig.~4, but \\textbf{without the bounding box interpolaion}.", fig.width=6, fig.height=4, outwidth="6in">>=
...
<<remove-single-province-genera-class-map, fig.width=6.45, fig.height=5.4, out.width="6.25in", fig.cap="Same as Fig.~3, but with \\textbf{genera occurring in single biogeographic provinces removed}.">>=
...
<<remove-single-province-genera-overall-map, fig.width=6, fig.height=4, out.width="6in", fig.cap="Same as Fig.~4, but with \\textbf{genera occurring in single biogeographic provinces removed}.">>=
...
labs.ns <- c(0.010, 0.012, 0.014, 0.016)
