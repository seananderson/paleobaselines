...
<<test-gbm1>>=
val_test <- rdply(200, validate_gbm(neog, shrinkage = 0.05,
...
<<relationship between mean risk and sampling intensity, fig.width=18, fig.height=9, out.width="7in", fig.cap="Log-transformed mean intrinsic risk of genera in a biogeographic province \\cite{spalding2007} versus number of OBIS records (of the indicated class) from that pronvince.  Each point represents a single biogeographic province. Lines and gray-shaded regions indicate linear regressions and 95\\% confidence intervals, colors indicate latitudinal zone of province.">>=
by.prov.classes2 <- by.prov.classes[(by.prov.classes$class != "Decapoda"),]
by.prov.classes2$log_OBIS_records <- log10(by.prov.classes2$OBIS_genera)
p <- ggplot(by.prov.classes2, aes(log_OBIS_records, mean.ext)) + facet_wrap(~class,scales="free",ncol=3) + stat_smooth(method = "lm", se = TRUE, size = .5,colour="black") + ylab("log(Mean intrinsic extinction risk)") + xlab("log10(Number of OBIS records)") + geom_point(data=by.prov.classes2,aes(log_OBIS_records, mean.ext,colour=Zone),alpha = 0.6) + scale_colour_manual(values=c("blue","green3","red"))
@
...
map_class_ext(er_dat_class3, yticks = exp(c(-0.5, 0, 0.5)),
  exact_limits = exp(c(-1, 1)), min_prov_genera = 10, col_range = TRUE, log_yticks = TRUE)
@
...
er_dat_class <- plyr::join(er_points, er@data, by = "id")
er_dat_class <- plyr::join(er_dat_class, by.prov.classes.no.singles, by = "PROV_CODE")
map_class_ext(er_dat_class, yticks = c(0.005, 0.01, 0.02, 0.05, 0.10, 0.20),
  min_prov_genera = 10, col_range = TRUE, exact_limits = c(0.0028,
    0.35), 
  ylabel = "Intrinsic extinction probability (over ~20 Myr)")
...
er_dat <- plyr::join(er_points, er@data, by = "id")
er_dat <- plyr::join(er_dat, by.prov.all.no.singles, by = "PROV_CODE")
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
labs <- seq(0.008, 0.016, 0.002)
labs_show <- rep(99, length(labs))
...
map_class_ext(er_dat_class, plot_column = "log.mean.gcd",
  ...
  yticks = c(12000, 16000, 20000), exact_limits = c(12000, 20000),
