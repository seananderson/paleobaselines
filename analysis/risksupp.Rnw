...
if(!file.exists("../data/val_test_1.0.rds")) {
  val_test_1.0 <- ldply(1:300, function(i)
...
if(!file.exists("../data/val_test_sing.rds")) {
  val_test_sing <- ldply(1:300, function(i)
...
#   val_test_singleton <- ldply(1:60, function(i)
...
# }
# val_test_sing <- val_test_singleton # for testing
@
...
# neog_subset <- FAKE_NEOG
  ...
  m_1.0 <- rlply(10, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
      max.lat + lat.range + mean.lat + great.circle + group,
...
if(!file.exists("../data/main_model_sing.rds")) {
  neog_subset <- neog[neog$single_obs == 0 & neog$prop_comp == 1.0, ]
#   neog_subset <- neog[neog$singleton == 0, ]
  w <- paleorisk::get_weights(neog_subset$Ex)
  weights <- ifelse(neog_subset$Ex == 1, w$ex_weight, w$sur_weight)
  m_sing <- rlply(10, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
      max.lat + lat.range + mean.lat + great.circle + group,
    data = neog_subset, n.trees = NTREES, interaction.depth = INT.DEPTH,
    distribution = "bernoulli", shrinkage = SHRINKAGE, weights = weights))
...
modern <- plyr::join(modern,
  ...
  by = c("class", "gbm_pred_binned"))
modern <- plyr::rename(modern, c("mean_observed" =
   "mean_observed_sing"))
modern$gbm_pred_binned <- NULL
...
#   picture_files = NULL, plot_column = "mean.ext.1.0",
  ...
  ylabel = "Intrinsic extinction probability (over ~23 Myr)")
dev.off()
# remove single occurrences in fossil record:
...
panel.cor.lower <- function(x, y, digits = 2, prefix = "", ...) {
  usr <- par("usr"); on.exit(par(usr))
  par(usr = c(0, 1, 0, 1))
  points(x, y, pch = 21, col = "#00000050", bg = "#00000030")
  abline(a = 0, b = 1, lty = 2, col = "blue")
}


pdf("../figs/pairs-paleo-culls-genera.pdf", width = 6, height = 6)
pairs(modern[,c("mean_observed", "mean_observed_1.0", "mean_observed_sing")],
  pch = 21, col = "#00000050", bg = "#00000010",
  upper.panel = panel.cor, lower.panel = panel.cor.lower)
dev.off()

pdf("../figs/pairs-paleo-culls-provinces.pdf", width = 6, height = 6)
