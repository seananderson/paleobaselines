...
<<plot-density-risk-by-province, fig.width=12.5, fig.height=15, fig.cap="Histograms showing the distribution of intrinsic extinction risk predictions for genera in each coastal biogeographic province \\cite{spalding2007} (panels) and taxonomic class (colors). Note that the x-axis is log-transformed.", echo=FALSE>>=
