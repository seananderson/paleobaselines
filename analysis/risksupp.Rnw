...
<<plot-genus-predictions-against-gcd2, fig.cap='Predicted intrinsic risk versus geographic range (great circle distance) for modern genera in each of six classes. The points represent individual genera and are colored according to taxonomic identity, the black contour lines indicate the density of genera, and the red line represents the mean intrinsic risk of all genera.', fig.width=9, fig.height=5.5>>=
