...
fit_models <- function(dat, test_fraction = 0.5, threshold = 0.5,
  ...
  interaction.depth = 1, n.trees = 300, bin = 0.025, smote = FALSE, ...) {
...
val_test <- rdply(100, fit_models(stcen, shrinkage = 0.1,
...
fit_stage_models <- function(dat, stage_name) {
  if(stage_name == "all") {
    ...
    d <- smolt(dat[dat$stage == stage_name, ])
  }
...
% \begin{figure}
%   \centering
%   \includegraphics[width=5.0in]{../figs/partial-plot-base.pdf}
%   \caption{Fig 1 from the main paper\ldots not completely finished with the new code and some of the echinoderms look suspect.}
%   \label{fig:partial-base}
% \end{figure}

\section{Merging risk predictions with modern species distributions}

Now we'll now use our GBM model to predict on the modern dataset:
...
modern$pred <- predict(m, newdata = modern, type = "response", n.trees = 300)
@
...
tropical_test <- reshape2::melt(by.prov.classes, id.vars =
  ...
  c("mean.lat.range", "mean.occupancy", "mean.gcd", "log_OBIS_records",
    "mean.richness"))
tropical_test <- join(tropical_test, by.prov.classes[, c("PROV_CODE",
