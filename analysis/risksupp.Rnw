...
d2 <- ddply(modern, "class", transform,
group_id = as.numeric(as.factor(as.character(as.numeric(group)))))

colourCount = length(unique(d2$group))
getPalette = colorRampPalette(brewer.pal(9, "Set1"))


ggplot(d2, aes(y = jitter(gbm_pred_binned, amount = 0.02), x = jitter(great.circle, amount = 800),colour=as.factor(group))) +  geom_hline(yintercept=mean(d2$gbm_pred_binned),colour="red") + geom_point(alpha=.6,pch=1,size=3) + facet_wrap(~class) + theme_bw() + xlab("Great circle distance") + ylab("Predicted intrinsic risk") + theme(strip.text.x = element_text(size = 8), axis.text = element_text(size = 8)) + theme(legend.text = element_text(size = 9)) + theme(legend.key.size = unit(.45, "cm")) + scale_colour_manual(values=c("red","blue","green","orange","red","blue","green3","deeppink1","orange","cyan","yellow3","cornflowerblue","darkorchid1","red","blue","green3","red","red","blue","green3","deeppink1","orange","cyan","yellow3","cornflowerblue","red","blue","green3")) + theme(legend.title=element_blank()) + theme(strip.text.x = element_text(size=14))+ stat_density2d(colour="black",size=.25,bins=16) + coord_cartesian(xlim=c(0,20800),ylim=c(0,1))
@
