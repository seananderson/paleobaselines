...
pkgs <- x[(other_line+1):(other_line + 3)]
@
...
stcen <- subset(stcen, class != "Malacostraca")
@
...
<<final-gbm-validation, eval=FALSE>>=
val_test <- rdply(200, validate_gbm(stcen, shrinkage = 0.1,
...
<<plot-gbm-no-mal, fig.width=7.5, fig.height=7, cache=FALSE>>=
ggplot(val_test_summarized,
  aes(gbm_pred_binned, mean_observed)) +
  geom_point(data = val_test, aes(gbm_pred_binned, obs_ext_prob),
...
stage_df <- expand.grid(stage_train = unique(stcen$stage),
  stage_test = unique(stcen$stage))
cross_pred <- mdply(stage_df, fit_interval_gbm, dat = stcen)
@
...
stage_models <- llply(c(stage_order, "all"), function(x)
  rlply(20, fit_stage_models(stcen, stage_name = x)))
...
groups_match <- data.frame(value =
  1:length(unique(stcen$group)),
  group = sort(unique(stcen$group)), stringsAsFactors = FALSE)
...
Now we'll now use our GBM model to predict on the modern dataset. We'll run our model a number of times and take the average predictions. This is because there is some stochasiticity to the GBM algorithm. TODO implement repeated prediction averaging.

<<build-main-paleo-model>>=
w <- paleorisk::get_weights(stcen$Ex)
weights <- ifelse(stcen$Ex == 1, w$ex_weight, w$sur_weight)

m <- rlply(20, gbm(Ex ~ richness + occupancy + occurrences + min.lat + 
    max.lat + lat.range + mean.lat + great.circle + class + group, 
  data = stcen, n.trees = 300, interaction.depth = 1,
  distribution = "bernoulli", shrinkage = 0.1, weights = weights))
@

Now we'll load the modern data:
...
And predict the modern intrinsic extinction risk from the paleo models. We'll take the average prediction across our multiple runs:
...
Then, we'll merge these predictions with our modern spatial observations. At the same time, we'll merge in Halpern et al. and Spalding et al.

Note which files this creates.
...
Now, let's inspect the role of tropical regions. We'll define tropical as\ldots

<<test-tropical-drivers, fig.width=16.5, fig.height=11.5, out.width="7in", cache=FALSE>>=
...
ggplot(tropical_test,
  aes(value, log(mean.ext), colour = Lat_Zone, shape = Lat_Zone)) +
