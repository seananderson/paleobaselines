...
# Recreate the singleton column:
# sing_test <- filter(stcen, !Interval_Name %in% c("Modern_raw", "Modern_merged", "Plio-Pleistocene")) %>%
#   sing_test <- stcen %>%
#   group_by(genus) %>%
#   mutate(n_stage = n_distinct(Interval_Name),
#     singleton2 = ifelse(n_stage == 1, 1, 0)) %>% as.data.frame
# table(sing_test[,c("singleton", "singleton2")])
# table(sing_test[,c("Ex", "singleton2")])
...
#   picture_files = NULL, plot_column = "mean.ext.1.0",
  silhouette_coords = silhouette_coords, max_range_given = exp(max_range)+1,
  ylabel = "Intrinsic extinction probability (over ~23 Myr)")
dev.off()
