...
cross_pred <- cross_pred[(cross_pred$stage_train != "Modern_raw" & cross_pred$stage_test != "Modern_raw" & cross_pred$stage_train != "Modern_merged" & cross_pred$stage_train != "Modern_merged"),]
...
Rsqs <- ddply(cross_pred,.(stage_train,stage_test),each(Rsq))
...
quartz()
p <- ggplot()
p + geom_point(data=cross_pred,aes(pred_self, pred, colour = class),alpha=0.3)  +
  ...
  geom_text(data=Rsqs,aes(X,Y,label=Rsq),size=3.5)
@
...
er_dat_class_no_single <- plyr::join(er_dat_class_nosingle, by.prov.classes.no.singles, by = "PROV_CODE")
