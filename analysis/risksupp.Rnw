...
\subsection{Standarizing predictor variables}
...
neog2 <- droplevels(subset(neog, stage_top < 23 &
    stage_top != 0))
@
...
if(!file.exists("../data/val_test.rds")) {
  val_test <- rdply(110, validate_gbm(neog, shrinkage = SHRINKAGE,
...
<<alternative-gbm-taxonomy-only-prep, fig.width=7, fig.height=4, out.width="6.25in", results='hide'>>=
...
<<alternative-gbms-plots, results='hide'>>=
