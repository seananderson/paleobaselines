...
<<check-classification-error, child='check-classification-error-child.Rnw', results='hide', fig.height=7, fig.width=14, fig.cap='Receiver operating characteristic (ROC) curves. ROC curves show sensitivity (true positive rate) plotted against specificity (true negative rate) across all possible probability thresholds. The area under the curves (AUC) represents the probability that a randomly chosen extinct genera will be ranked more at risk than a randomly chosen genera that surves. The dashed line represents the the 50\\% line --- predictions on this line would be no better than tossing a coin. We show predictions for models built on the entire Neogene (as used in Fig.~2 and Fig.~3) and for models built on stages within the Neogene. The coloured lines represent ROC curves from the same models but split out by major taxonomic groups.'>>=
@
