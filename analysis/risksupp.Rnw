...
Now, we will run many more cross-validation iterations of our model to get precise estimates of non-linear bias so that we can use the observed mean extinction probabilities as calibration factors:
...
Now we can compare the predictions made from other stages with the predictions from the same stage. This code produces Figure~\ref{fig:plot-cross-interval-gbms}.
...
Now, we will fit the models for each stage. Within each stage we will fit the model a number of times:
...
Now we will get the median and interquartile range of the marginal effects.
...
Now we will use our calibration factors on our predictions.
