...
Sampling in the modern is patchy.

We assume that if a genus is identified within a biogeographic realm \cite{spalding2007}, it is likely to be found throughout that realm.
...
This is similar to the method used in Ref.~\cite{belanger2012}.

We didn't interpolate the marine mammals or elasmobranch datasets since these were based on range maps.

We split gaps of $90\,^{\circ}$ longitude or greater into separate bounding boxes.
