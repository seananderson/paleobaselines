...
This document is written with the \pkg{knitr} package for the \proglang{R} statistical environment. It can be re-created by running \fn{knitr::knit2pdf()} on the \texttt{.Rnw} file. A number of functions that are used in this analysis have been compiled into an \proglang{R} package \pkg{paleorisk}. To start, we will load this package:

<<load-this-package>>=
library("paleorisk")
@

Now we'll load other packages that are needed for our analysis:

<<load-other-packages, cache=FALSE, eval=TRUE>>=
library("plyr")
library("dplyr")
library("reshape2")
library("ggplot2")
...
pkgs <- x[(other_line+1):(other_line + 4)]
@

We used \Sexpr{rversion} and package versions

<<versions, echo=FALSE, results=TRUE>>=
pkgs
@

\section{Standardizing the predictors}

We will start by standardizing a variety of habitat-relative characteristics within each geologic period. This is an important step to make some habitat characteristics (richness, occupancy, occurences) comparable across periods, an in particular, between the paleontological data and the modern data. Given the substantially different sampling methods within the paleontological and modern records, the absolute values of richess, occupancy, and occurences are not comparable. We therefore standardize them to values between 0 and 1 within each interval. The other variables (e.g.\ latitude variables and great circle distance) are still comparable between the paleontological and modern records --- a degree latitude is still a degree latitude and kilometer is still a kilometer.

The following script creates the \proglang{R} data object \texttt{standardized-predictors-cenozoic-obis.rds}.
...
We'll load the standardized data and subset it for the Neogene period. The modern data are also contained in this data frame with \texttt{stage\_top = 0}. Therefore, we will also exclude the modern dataset from what we call \texttt{stcen} for standardized Cenozoic.
...
<<test-gbm1, cache=TRUE>>=
val_test <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
  interaction.depth = 1, n.trees = 300, use_weights = TRUE)$bin_validation)
@

Next, we'll compute some summary statistics for the observed extinction probability at each bin across our replicates. We'll use the \fn{summarize\_val\_test()} from our \pkg{paleorisk} package. This computes means and 95\% confidence intervals (based on likelihood profiles)for each validation bin.
...
val_test_base <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
    interaction.depth = 1, n.trees = 300, use_weights = TRUE)$bin_validation)
val_test_shr <- rdply(50, validate_gbm(stcen, shrinkage = 0.01,
    interaction.depth = 1, n.trees = 300, use_weights = TRUE)$bin_validation)
val_test_int <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
    interaction.depth = 4, n.trees = 300, use_weights = TRUE)$bin_validation)
val_test_ntrees <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
...
We note that Malacostraca is poorly predicted in our out-of-sample validation tests. We therefore choose here to not include Malacostraca in the model that we will predict the modern intrinsic extinction risk with.

<<drop-malacostraca, cache=FALSE>>=
stcen2 <- gdata::drop.levels(subset(stcen, class != "Malacostraca"))
@

Now, we'll run many more cross-validation iterations of our model to get precise estimates of non-linear bias so that we can use the observed mean extinction probabilities as calibration factors.

<<final-gbm-validation, eval=TRUE, cache=TRUE>>=
val_test <- rdply(200, validate_gbm(stcen2, shrinkage = 0.1,
  interaction.depth = 1, n.trees = 300,
  use_weights = TRUE)$bin_validation)
@

<<final-gbm-summarize, eval=TRUE, cache=TRUE>>=
val_test_summarized <- ddply(val_test, c("class", "gbm_pred_binned"),
  summarize_val_test)
@

And we'll plot these validation values again:

<<plot-gbm-no-mal, fig.width=8, fig.height=5, cache=FALSE>>=
ggplot(val_test_summarized,
  aes(gbm_pred_binned, mean_observed)) +
  geom_point(data = val_test, aes(gbm_pred_binned, obs_ext_prob),
...
Now, we can use these observed extinction probabilities (in \texttt{val\_test\_summarized}) as calibration for our GBM-predicted extinction probability in the modern dataset. To do this, we take the predicted extinction probability from our GBM model applied to the modern dataset, we fit that value into our binning scheme, and we match it up with the empirically-observed mean extinction probability for that bin. This removes the observed non-linear bias in our model \cite{thorson2012}. Put another way, we find our predicted value on the x-axis of the last plot and draw a vertical line up until we meet a blue dot. We then use the value of the blue dot as our calibrated extinction probability. We'll come back to that shortly when we apply our model to the modern dataset.

\section{Testing predictive performance across intervals}

Another way to test the predictive power of our models is to build the model on one geologic interval and predict on another. To keep this simple, we won't use our calibration factors.

<<cross-intervals-gbms, cache=TRUE>>=
...
Because there's some stochasticity when running a GBM, we'll run our models a number of times, and each time record the marginal effects. We'll then take the median and interquartile range of those predictions.
...
Now, we'll fit the models for each stage. Within each stage we'll fit the model a number of times:

<<fit-marginal-stage-models, cache=TRUE>>=
...
And, we'll go through the stage-based models and pull out the partial dependence data:

<<pull-out-marginal-data, cache=TRUE>>=
...
Now we'll get the median and interquartile range of the marginal effects. We'll use the \pkg{dplyr} package, which is slightly faster than \pkg{plyr}:

<<summarize-marginal-effects, cache=TRUE>>=
...
And we'll make some quick plots of the data:
...
We'll use our GBM model to predict on the modern dataset. We'll run our model a number of times and take the average predictions. This is because there is some stochasticity to the GBM algorithm.
...
Next, we'll load the modern data:
...
And predict the modern intrinsic extinction risk from the paleontological models. We'll take the average prediction across our multiple runs:
...
Now we'll use our calibration factors on our predictions. We're going to set any predictions in which the median sample number of observations in our calibration is 1 or less to \texttt{NA}.

<<show-calibration-NA-cutoff, fig.height=4>>=
ggplot(val_test_summarized, aes(mean_observed, median_sample_n)) + 
  geom_point() + facet_wrap(~class, scales = "free_y") + 
  geom_abline(intercept = 1, slope = 0)
@


<<calibrate-risk-modern>>=
val_test_summarized[val_test_summarized$median_sample_n <= 1, 
  "mean_observed"] <- NA
...
We'll also save a version of our predictions where we build the predictions based on the average predictions across geological stage models. We've already built these models. We'll take the average predictions across replicated fits and geological stages.
...
We'll save the \texttt{modern} data frame for future use.
...
<<compare-neogene-gbm-to-stage-models>>=
...
Next, we'll merge these predictions with our modern spatial observations. At the same time, we'll merge in Halpern et al. and Spalding et al.

TODO Note which files this creates.

<<merge-predictions-spatial, cache=FALSE>>=
source("merge-predictions-to-map.R")
@
...
ggplot(d.eco.filled, aes(mean_observed + 0.001)) + 
  geom_density() + facet_wrap(~class) + scale_x_log10() +
  ylab("Density") + xlab("Predicted extinction probabilty")
@

And the distribution of our predictions by province and class:

<<plot-density-risk-by-province, fig.width=15, fig.height=12, out.width="7in">>=
...
ggplot(temp, aes(mean_observed + 0.001)) + 
  geom_density(aes(fill = class, colour = class), 
    position = "stack", alpha = 0.5) + 
  facet_wrap(~ PROV_pretty, scales = "free_y") + 
  scale_x_log10() +
  ylab("Density") + xlab("Predicted extinction probabilty")
@
...
Now, let's inspect the role of tropical regions. Earlier we defined ``tropical'' as less than $30\deg$ absolute latitude.

<<test-tropical-drivers, fig.width=18, fig.height=11.5, out.width="7in", cache=FALSE>>=
