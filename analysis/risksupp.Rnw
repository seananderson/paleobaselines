...
This document is written with the \texttt{knitr} package for the \texttt{R} statistical environment. It can be re-created by running \texttt{knitr::knit2pdf()} on the \texttt{.Rnw} file. A number of functions that are used in this analysis have been compiled into an \texttt{R} package \texttt{paleorisk}. To start, we will load this package:
...
pkgs <- x[(other_line+1):(other_line + 2)]
@
...
We'll also create a function to implement SMOTE upsampling on our data. TODO what SMOTE is and why\ldots
...
val_test_int <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
    interaction.depth = 4, n.trees = 300)$bin_validation)
val_test_ntrees <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
    interaction.depth = 1, n.trees = 1000)$bin_validation)
@
...
Now, we'll run many more cross-validation iterations to get more precise estimates of non-linear bias so that we can use the observed mean extinction probabilities as calibration factors.

% Note: manually cached chunks:
% Switch to eval=FALSE and the next chunk to eval=TRUE to re-run
% the many-repetition version. This will, of course, take a long time.
<<final-gbm-validation-load, eval=TRUE, echo=FALSE>>=
load("../data/val_test.rda")
load("../data/val_test_summarized.rda")
@

<<final-gbm-validation, eval=FALSE>>=
val_test <- rdply(50, validate_gbm(stcen, shrinkage = 0.1,
  interaction.depth = 1, n.trees = 300, smote = FALSE, 
  use_weights = TRUE)$bin_validation)
val_test_summarized <- ddply(val_test, c("class", "gbm_pred_binned"),
  summarize_val_test)
@

<<cache-final-gbm-validation, echo=FALSE>>=
save(val_test, file = "../data/val_test.rda")
save(val_test_summarized, file = "../data/val_test_summarized.rda")
@

<<plot-gbm-no-mal, fig.width=7.5, fig.height=7, fig.cap="Observed vs.\\ predicted extinction probability with the model repeatedly built on 50\\% of the data and tested on the other 50\\% with the SMOTE sampling algorithm implemented. See Figure~\\ref{fig:test-gbm1-bias-plot} for a full figure description.">>=
ggplot(val_test_summarized,
  aes(gbm_pred_binned, mean_observed)) +
  geom_point(data = val_test, aes(gbm_pred_binned, obs_ext_prob),
    alpha = 0.1) +
  geom_linerange(aes(ymin = l, ymax = u), colour = "#4E9DF0") +
  geom_point(aes(size = median_sample_n), colour = "#4E9DF0") +
  facet_wrap(~class) + geom_abline(intercept = 0, slope = 1/4) +
    facet_wrap(~class) + geom_abline(intercept = 0, slope = 1) +
  scale_size(trans = "log10") + coord_fixed(ratio = 1) +
  ylim(0, 1) + xlim(0, 1) +
  xlab("Out-of-sample GBM-predicted extinction probability") +
...
Another way to test the predictive power of our models is to build the model on one geologic interval and predict on another. For the purposes of this comparison, we are not going to implement the SMOTE algorithm here.

<<cross-intervals-gbms>>=
stage_df <- expand.grid(stage_train = unique(stcen$stage),
  stage_test = unique(stcen$stage))
cross_pred <- plyr::mdply(stage_df, fit_interval_gbm, dat = stcen)
@
...
TODO implement averaging of the partial dependence values to average across gbm() and SMOTE algorithms
...
fit_stage_models <- function(dat, stage_name) {
  if(stage_name == "all") {
    ...
    d <- smote(dat[dat$stage == stage_name, ])
  }
...
stage_models <- llply(c(stage_order, "all"), function(x)
  fit_stage_models(stcen, stage_name = x))
...
partial_dat <- ldply(c(stage_order, "all"), function(stage_name) {
  p2 <- ldply(vars, function(var_name){
    p1 <- gbm::plot.gbm(stage_models[[stage_name]], return.grid = TRUE,
...
Now we'll now use our GBM model to predict on the modern dataset. We'll run out model a number of times and take the average predictions. This is because there is some stochasiticity to the SMOTE algorithm and the GBM algorithm. TODO implement repeated prediction averaging.

<<predict-risk-modern>>=
m <- gbm(Ex ~ richness + occupancy + occurrences + min.lat + max.lat +
  lat.range + mean.lat + great.circle + class + group, data =
  smote(stcen), n.trees = 300, interaction.depth = 1,
  distribution = "bernoulli", shrinkage = 0.1)
temp_dat <- readRDS("../data/standardized-predictors-cenozoic-obis.rds")
modern <- subset(temp_dat, class != "Testudines" & stage == "Modern")
modern$pred <- gbm::predict.gbm(m, newdata = modern, type = "response", 
  n.trees = 300)
####

# smote_ratio <- function(x) {
#   round(table(x$class, x$Ex)/rowSums(table(x$class, x$Ex)) * 100)
# }
# smote_ratio(stcen)
# smote_ratio(stcen_smote)
