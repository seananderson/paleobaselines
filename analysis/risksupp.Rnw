...
# only p > 0.25:
if(!file.exists("../data/val_test_0.25.rds")) {
  val_test_0.25 <- rdply(1000, validate_gbm(neog[neog$prop_comp > 0.25, ],
    shrinkage = SHRINKAGE,
    interaction.depth = INT.DEPTH, n.trees = NTREES,
    use_weights = TRUE)$bin_validation,
    .progress = "text")
  saveRDS(val_test_0.25, file = "../data/val_test_0.25.rds")
} else {
  message(paste("Loading previous GBM cross-validations.",
    "Delete ../data/val_test_0.25.rds to rerun these."))
  val_test_0.25 <- readRDS("../data/val_test_0.25.rds")
}
# only p > 0.5:
if(!file.exists("../data/val_test_0.50.rds")) {
  val_test_0.50 <- rdply(1000, validate_gbm(neog[neog$prop_comp > 0.50, ],
    shrinkage = SHRINKAGE,
    interaction.depth = INT.DEPTH, n.trees = NTREES,
    use_weights = TRUE)$bin_validation,
    .progress = "text")
...
val_test_summarized <- ddply(val_test, c("class", "gbm_pred_binned"),
  summarize_val_test)
val_test_summarized_0.25 <- ddply(val_test_0.25, c("class", "gbm_pred_binned"),
  summarize_val_test)
...
# val_test_plot <- val_test_0.25[sample(1:nrow(val_test_0.25), size = 10000), ]
# p <- ggplot(val_test_summarized_0.25,
#   aes(gbm_pred_binned, mean_observed)) +
#   geom_point(data = val_test_plot, aes(gbm_pred_binned, obs_ext_prob),
#     alpha = 0.08) +
#   geom_linerange(aes(ymin = l, ymax = u), colour = "#4E9DF0") +
...
#   ylab("Observed scaled extinction probability") +
#   labs(size = "Median sample #")
# ggsave("../figs/test-gbm1-bias-plot-25.pdf", width = 9, height = 5)
@
...
# 0.25 prop complete threshold:
if(!file.exists("../data/main_model_0.25.rds")) {
  neog_subset <- neog[neog$prop_comp > 0.25, ]
  w <- paleorisk::get_weights(neog_subset$Ex)
  weights <- ifelse(neog_subset$Ex == 1, w$ex_weight, w$sur_weight)
  m_0.25 <- rlply(10, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
      max.lat + lat.range + mean.lat + great.circle + group,
    data = neog_subset, n.trees = NTREES, interaction.depth = INT.DEPTH,
    distribution = "bernoulli", shrinkage = SHRINKAGE, weights = weights))
  saveRDS(m_0.25, file = "../data/main_model_0.25.rds")
} else {
  m_0.25 <- readRDS("../data/main_model_0.25.rds")
}
# 0.50 prop complete threshold:
if(!file.exists("../data/main_model_0.50.rds")) {
  neog_subset <- neog[neog$prop_comp > 0.50, ]
  w <- paleorisk::get_weights(neog_subset$Ex)
  weights <- ifelse(neog_subset$Ex == 1, w$ex_weight, w$sur_weight)
  m_0.50 <- rlply(10, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
      max.lat + lat.range + mean.lat + great.circle + group,
    data = neog_subset, n.trees = NTREES, interaction.depth = INT.DEPTH,
    distribution = "bernoulli", shrinkage = SHRINKAGE, weights = weights))
  saveRDS(m_0.50, file = "../data/main_model_0.50.rds")
} else {
  m_0.50 <- readRDS("../data/main_model_0.50.rds")
}
...
predictions <- laply(m, function(x)
  gbm::predict.gbm(x, newdata = modern, type = "response",
    n.trees = NTREES))
predictions_0.25 <- laply(m_0.25, function(x)
  gbm::predict.gbm(x, newdata = modern, type = "response",
    n.trees = NTREES))
predictions_0.50 <- laply(m_0.50, function(x)
  gbm::predict.gbm(x, newdata = modern, type = "response",
    n.trees = NTREES))
modern$pred <- apply(predictions, 2, mean)
modern$pred_0.25 <- apply(predictions_0.25, 2, mean)
modern$pred_0.50 <- apply(predictions_0.50, 2, mean)
@

% Use our calibration factors on our predictions:
<<calibrate-risk-modern>>=
modern$gbm_pred_binned <- assign_bins(modern$pred_0.25)
modern <- plyr::join(modern,
  val_test_summarized[, c("class", "gbm_pred_binned", "mean_observed")],
  by = c("class", "gbm_pred_binned"))
modern <- plyr::rename(modern, c("mean_observed" = "mean_observed_0.25"))
modern$gbm_pred_binned <- NULL

modern$gbm_pred_binned <- assign_bins(modern$pred_0.50)
modern <- plyr::join(modern,
  val_test_summarized[, c("class", "gbm_pred_binned", "mean_observed")],
  by = c("class", "gbm_pred_binned"))
modern <- plyr::rename(modern, c("mean_observed" = "mean_observed_0.50"))
modern$gbm_pred_binned <- NULL
...
% And make comparison versions with the paleo culls of > 0.25 and 0.5 prop
% complete:
...
# class maps:
# 0.25 cull:
pdf("../figs/class-risk-maps-mean-log-ext-paleo-25.pdf", width = 6.45, height = 5.1)
max_range <- map_class_ext(er_dat_class, yticks = c(0.005, 0.01, 0.02, 0.05, 0.10),
  min_prov_genera = 10, col_range = TRUE, exact_limits = c(0.0034, 0.15),
  picture_files = picture_files, plot_column = "mean.ext.25",
  silhouette_coords = silhouette_coords,
  ylabel = "Intrinsic extinction probability (over ~23 Myr)")
dev.off()
# 0.50 cull:
pdf("../figs/class-risk-maps-mean-log-ext-paleo-50.pdf", width = 6.45, height = 5.1)
max_range <- map_class_ext(er_dat_class, yticks = c(0.005, 0.01, 0.02, 0.05, 0.10),
  min_prov_genera = 10, col_range = TRUE, exact_limits = c(0.0034, 0.15),
  picture_files = picture_files, plot_column = "mean.ext.50",
  silhouette_coords = silhouette_coords,
  ylabel = "Intrinsic extinction probability (over ~23 Myr)")
dev.off()
...
# and the overall map:
# 0.00 cull:
pdf("../figs/hotspots-paleo-00.pdf", width = 7, height = 3.5)
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
...
dev.off()
# 0.25 cull:
pdf("../figs/hotspots-paleo-25.pdf", width = 7, height = 3.5)
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
map_hotspots(er_dat, min_prov_genera = 100, add_legend = TRUE,
  hotspots = TRUE, at = log(labs), at.labels = labs_show,
  plot_column = "mean.ext.25", col_limits = COL_LIMITS)
par(xpd = NA)
mtext("Intrinsic extinction probability", side = 4, outer = TRUE,
  line = -1.4, las = 0, col = "grey30", cex = 0.9)
dev.off()
# 0.50 cull:
pdf("../figs/hotspots-paleo-50.pdf", width = 7, height = 3.5)
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
map_hotspots(er_dat, min_prov_genera = 100, add_legend = TRUE,
  hotspots = TRUE, at = log(labs), at.labels = labs_show,
  plot_column = "mean.ext.50", col_limits = COL_LIMITS)
par(xpd = NA)
mtext("Intrinsic extinction probability", side = 4, outer = TRUE,
  line = -1.4, las = 0, col = "grey30", cex = 0.9)
dev.off()
...
pairs(modern[,c("mean_observed", "mean_observed_0.25", "mean_observed_0.50")],
  pch = 21, col = "#00000050", bg = "#00000010",
  upper.panel = panel.cor)
dev.off()

pdf("../figs/pairs-paleo-culls-provinces.pdf", width = 6, height = 6)
pairs(by.prov.all[,c("mean.ext", "mean.ext.25", "mean.ext.50")],
  pch = 21, col = "#00000050", bg = "#00000010",
  upper.panel = panel.cor)
dev.off()
@
