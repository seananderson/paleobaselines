...
dat <- readRDS(data, file = "../data/modern_and_paleo_ranges.rds")
Genera <- function(df) length(unique(df$genus))
Occurrences <- function(df) sum(df$occurrences)
Table.S1 <- plyr::ddply(dat, plyr::.(Interval,class),
  plyr::each(Genera, Occurrences))
Table.S1 <- Table.S1[(Table.S1$Interval != "Modern_merged" &
  Table.S1$class != "Malacostraca" &
  Table.S1$class != "Foraminifera" &
  Table.S1$class != "Testudines"), ]
Table.S1$Interval <- plyr::revalue(Table.S1$Interval, c("Modern_raw" = "Modern"))
Table.S1$Interval <- factor(Table.S1$Interval,
  levels = c("Lower Miocene", "Middle Miocene",
    "Upper Miocene","Plio-Pleistocene", "Modern"))
Table.S1 <- Table.S1[order(Table.S1$Interval, Table.S1$class), ]
...
neog <- droplevels(subset(neog, neog$stage_top < 23 &
    neog$stage_top != 0))
@
...
if(!file.exists("../data/val_test.rds")) {
  val_test <- rdply(200, validate_gbm(neog, shrinkage = SHRINKAGE,
...
<<check-within-interval-performance, results='hide'>>=
...
<<plot-within-stage-tests, fig.width=8.5, fig.height=7, out.width="6.25in", fig.cap="TODO fix caption. Within interval TODO-fold cross-validation prediction. Fit shown is a logistic regression. (probability with weights for numbers of samples).">>=
...
Another way to test the predictive power of our models is to build the model on one geologic stage and predict on another. We will not use our calibration factors in this test to keep the code as simple as possible and to minimize computational time. We can compare the predictions made from other stages with the predictions from the same stage (Fig.~2). We see that in general there is substantial similarity in extinction regimes between succeeding geologic stages with respect to the evaluated taxonomic and geographic range predictor variables. TODO: flesh this out a bit more
...
# comparisons:
Rsq <- function(df) {
  Rsq <- cor(df$pred,df$pred_self,method="spearman")^2
  return(Rsq)
}
...
<<alternative-gbm-taxonomy-only-prep, fig.width=7, fig.height=4, out.width="6.25in">>=
...
<<obis-residuals-maps, fig.width=6.45, fig.height=5.4, out.width="6.25in", fig.cap="TODO add to this --- residuals after accounting for OBIS sampling.">>=
...
er_dat_class3  <- merge(er_dat_class2, resids,
  by = c("class","PROVINCE"))
...
<<remove-single-province-genera-class-map, result='hide'>>=
...
\begin{center}
  \includegraphics[width=4.86in]{../figs/plot-cross-interval-gbms.pdf}\\
  \smallskip
  Figure 2
\end{center}
\clearpage
