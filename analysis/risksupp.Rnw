...
ggplot(temp, aes(mean_observed + 0.001)) +
  ...
  scale_x_log10() + theme(strip.text.x = element_text(size = 7)) +
...
One way to test the sensitivity of our aggregate map (Fig.~3) is to create the map multiple times, each time leaving out one taxonomic class in succession. This is often referred to as a jackknife analysis and can be an effective way to visually present uncertainty \cite{branch2010}. The plot gives us an idea if any one class is driving the pattern. The jackknifed figure is shown in Fig.~\ref{fig:jackknife-map}.

We see that the Caribbean and Antarctic hotspots are driven primarily by gastropods. Many hotspots in the Indo-Pacific remain regardless of removing any one taxonomic class.
...
TODO: discuss the plot of number of occurrences per province above, the crossplot of number of occurrences vs. mean risk, and the use of the latter relationships to detrend and produce the residuals map

TODO Although the correspondence

We also considered class-level maps (a version of Fig.~2) after accounting for the relationship between OBIS sampling and predicted extinction probability shown in Fig.~\ref{fig:plot-n-genera-prov}. To do this, we mapped the residuals from the regressions in Fig.~\ref{fig:plot-n-genera-prov}. The resulting maps are shown in Fig.~\ref{fig:obis-residuals-maps}.

TODO discuss these maps
...
As another test, we tried removing genera that were only reported in one province. This removes \Sexpr{single.genera.provs.removed} from a total of \Sexpr{total.genera} genera (Fig.~\ref{fig:remove-single-province-genera}).
...
<<min-gen-thresh-sensitivity, fig.height=8.5, fig.width=13, fig.cap='Effect of raising the minimum genus richness cutoff  provinces on the global distribution of mean intrinsic risk (a cutoff of 50 genera is used in Figure 3). At low cutoff values, island provinces with low sampled diversity tend to dominate the relative risk pattern. This is primarily attributable to differential sampling effects: even poorly-sampled provinces have a complete inventory of mammal and shark genera because the distributions of genera are based on published range maps not OBIS occurrences. Since mammals have very high intrinsic risk relative to most other groups, the high ratio of mammal genera to other genera gives leads to exceptionally high mean intrinsic risk estimates in poorly sampled provinces. Removing provinces that are clearly badly undersampled ameliorates this effect. The color scale is adjusted to represent the full range of variation shown in each map.'>>=
