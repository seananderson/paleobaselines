...
In the second case, we built models on individual stages and then compared those predictions from predictions made by models from other stages (Fig.~\ref{fig:plot-cross-interval-gbms}). This is one test of the consistency of the determinants of extinction risk through time. We see that one stage can predict a considerable proportion of variability in predicted extinction probability from another stage, with Spearman's rank correlation coefficients ranging from TODO to TODO.
...
We also identified the relative contribution of taxonomy vs.\ range and occupancy characteristics. To do this, we examined the relative importance of each of the GBM predictors using the \texttt{relative.influence()} function in the \pkg{gbm} package, which uses the algorithm as described in Ref.~\cite{friedman2001}.
