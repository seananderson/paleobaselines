...
%The \proglang{R} package itself can be installed (from the \texttt{analysis} folder) by running:
...
\subsection{Standardizing the predictors}
...
# global_45x14$longitude
# global_45x14$latitude
...
We will start by standardizing a variety of habitat-relative characteristics within each geologic stage. All of the geographic range, specied richness, and occurrence frequency predictors are potentially influenced by sampling intensity, and the modern dataset is much larger in terms of total number of occurrences than the paleontological datasets (Table S1).  Metrics that are geographically explicit (max.lat, min.lat, mean.lat, lat.range, great.circle) cannot easily be standardized without distorting underlying patterns, but those that are not (richness, occurrences, occupancy) were divided by the maximum value observed in each interval. Hence, the scale on these standardized predictors goes from 0 to 1 (Figure 1). The other variables (e.g.\ latitude variables and great circle distance) are still comparable between the paleontological and contemporary records --- a degree latitude is still a degree latitude and a kilometre is still a kilometre.
...
#drop genera that occur only once in the entire database if desired
#dat <- dat[(dat$singleton.single.occ == 0),]
#now remove the extraneous columns that were created only for culling
dat <- dat[ , -which(names(dat) %in% c("Singleton","singleton.single.occ"))]
...
# set occurrence cutoffs for modern and paleo data
out <- list()
for(i in 1:length(unique(dat$Interval_Name))) {
  out[[i]] <- standardize_data(dat, interval = unique(dat$Interval_Name)[i])
}
out <- do.call("rbind", out)
out$Interval_Name <- out$stage
saveRDS(out, file = "../data/stand-predictors-cen-obis.rds")
@

%TODO: update this table so that it isn't operating on standardized occurrences
<<genera-occurrences-table, results = "asis", eval=FALSE>>=
...
% TODO - this plot needs to have culling done first or use real data
<<plot-comparison-modern-plio-pleistocene-ranges, out.width="\\textwidth", fig.height=5, fig.width=8, fig.cap=paste("Comparison of ranges for the", length(unique(Plio.Modern2$genus)), "genera that are sampled in both the modern dataset and in the Plio-Pleistocene subset of the paleontological dataset. Richness, occupancy, and occurrences are standardized by dividing by the maximum observed value within each interval. Points are overplotted and hence relative tranparancy of a given point indicates the number of genera. Black lines are lines of unity and red lines are reduced major axis regression lines."), eval=FALSE>>=
Plio.Modern <- readRDS("../data/stand-predictors-cen-obis.rds")
Plio.Modern <- Plio.Modern[Plio.Modern$Interval_Name %in% 
  c("Modern_raw", "Plio-Pleistocene"), ]
Plio.Modern$tropical_only <- NULL
Plio.Modern$Ex <- NULL
Plio.Modern <- reshape2::melt(Plio.Modern, 
  id = c("Interval_Name","stage","stage_top","class","group","genus"))
...
Plio$stage <- NULL
Plio$stage_top <- NULL
Plio.Modern2 <- merge(Modern,Plio, 
  by = c("class","group","genus","variable"),all.x=FALSE,all.y=FALSE)
...
# can throw out less than 23 - neogene only
neog <- droplevels(subset(neog, stage_top < 23 &
    stage_top != 0))
@
...
if(!file.exists("../data/val_test.rds")) {
  val_test <- rdply(40, validate_gbm(neog, shrinkage = SHRINKAGE,
...
<<plot-alt-gbms, fig.height=8, fig.width=13, out.width="7in", echo=FALSE, eval=FALSE>>=
ggplot(val_test_alt,
  aes(gbm_pred_binned, mean_observed)) +
...
<<check-within-interval-performance, results='hide', eval=FALSE>>=
stage_order <- c("Lower Miocene", "Middle Miocene", "Upper Miocene",
  "Plio-Pleistocene")
out <- list()
for(i in 1:length(stage_order)) {
  message(stage_order[[i]])
  out[[i]] <- rdply(75, validate_gbm(
      subset(neog, Interval_Name == stage_order[i]), 
      shrinkage = SHRINKAGE, interaction.depth = INT.DEPTH, 
      n.trees = NTREES, use_weights = TRUE, 
      test_fraction = 0.3333)$bin_validation,
      .progress = "text")
  out[[i]]$stage <- stage_order[[i]]
}
within_stage <- do.call("rbind", out)

oos_rsq_glm <- plyr::ddply(within_stage, "stage", function(x) {
  require(mgcv)
  m <- mgcv::gam(obs_ext_prob ~ gbm_pred_binned, data = x, 
    weights = sample_n, family = binomial)
  summary(m)$dev.expl
  })

glm_pred <- plyr::ddply(within_stage, "stage", function(x) {
   m <- glm(obs_ext_prob ~ gbm_pred_binned, data = x, 
     weights = sample_n, family = binomial)
   newdat <- data.frame(gbm_pred_binned = seq(min(x$gbm_pred_binned),
       max(x$gbm_pred_binned), length.out = 100))
   data.frame(gbm_pred_binned = newdat$gbm_pred_binned, 
     pred = predict(m, type = "response", newdata = newdat))
   })
@

<<plot-within-stage-tests, fig.width=8.5, fig.height=7, out.width="6.25in", fig.cap="TODO fix caption. Within interval TODO-fold cross-validation prediction. Fit shown is a logistic regression. (probability with weights for numbers of samples).", eval=FALSE>>=
ggplot(data = within_stage) +
  geom_point(aes(x = gbm_pred_binned, 
      y = obs_ext_prob, size = sample_n, colour = class), alpha = 0.2) +
  facet_wrap(~stage) +
  scale_size(trans = "log10") + coord_fixed(ratio = 1) +
  ylim(0, 1) + xlim(0, 1) +
  xlab("Out-of-sample GBM-predicted scaled extinction probability") +
  ...
  labs(size = "Sample #") + theme_bw() +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  geom_line(data = glm_pred, aes(gbm_pred_binned, pred), 
    col = "darkgrey", lwd = 1) +
  geom_text(data = oos_rsq_glm, aes(.05,.90, 
      label = sprintf("%.2f", V1)), size=3.5)
@

%TODO re-write this: Proportion of null deviance explained from a GLM fitted to the binned predictions against observed extinctions. Fitted to probabilities with weights for number of samples. Using \pkg{mgcv} but without smooth terms. 
...
<<plot-cross-interval-gbms, results='hide'>>=
...
newclass <- data.frame(c("Gastropoda","Bivalvia","Anthozoa","Echinoidea","Elasmobranchii","Mammalia"),c("Gastropoda","Bivalvia","Scleractinia","Echinoidea","Sharks","Mammalia"))
...
# make plot
pdf("../figs/plot-cross-interval-gbms.pdf", width = 9, height = 7)
p <- ggplot()
p <- p + geom_point(data=cross_pred2,
    aes(pred_self, pred, colour = class2), alpha=0.25, pch=16)  +
  facet_grid(stage_train ~ stage_test) +
  scale_y_log10() + scale_x_log10() +
  scale_colour_brewer(palette = "Set2",breaks=c("Scleractinia","Bivalvia","Gastropoda","Echinoidea","Sharks","Mammalia")) +
  #scale_colour_manual(values = c("blue","yellow2","skyblue2","red","limegreen","chocolate2"),breaks=c("Scleractinia","Bivalvia","Gastropoda","Echinoidea","Sharks","Mammalia")) +
  xlab("Scaled extinction probability (stage that model was built on)") +
  ylab("Scaled extinction probability (stage that model was tested on)") +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  geom_text(data=Rsqs,aes(.005,.70,label=sprintf("%.2f", Rsq)),size=3.5) +
  theme(panel.grid.major = element_blank()) + 
  theme(panel.grid.minor = element_blank()) +
  theme(legend.title=element_blank()) 
print(p)
dev.off()
# Cross-interval comparison of genus risk estimates from generalized boosting models. The x-axis shows the probability of extinction as predicted by a model built on data from the same stage. The y-axis shows the probability of extinction as predicted by a model built on data from a different stage. The colours show different taxonomic classes. Note the log-distributed x- and y-axes. Since there is some stochasticity involved in the generalized boosting machine fitting routine, there is some variability in predictions within the same stage (the diagonal panels). In general, there is substantial similarity in extinction regimes between succeeding intervals with respect to the evaluated taxonomic and geographic range predictor variables.
@

<<make-cross-pred-plot-baseplot, results='hide'>>=
...
par(mfrow = c(4,4), mar = c(0,0,0,0), cex = 0.5, oma = c(3.8, 4.0, 4, 2), mgp = c(2, 0.4, 0), tck = -0.02)
par(las = 1)
lim_lower <- min(c(cross_pred$pred_self, cross_pred$pred))
i <<- 0
plyr::d_ply(cross_pred2, c("stage_train", "stage_test"), function(x) {
  i <<- i + 1
  with(x, plot(pred_self, pred, col = cols_outer, bg = cols_inner, pch = 21, axes = FALSE, log = "xy", xlim = c(lim_lower, 1), ylim = c(lim_lower, 1), yaxs = "i", xaxs = "i"))
  box(col = "grey50", lwd = 0.8)
  if(i %in% seq(1, 13, 4)) axis(2, at = c(0.01, 0.1, 1), col = "grey50", col.axis = "grey50", lwd = 0.8)
  if(i %in% seq(13, 16)) axis(1, at = c(0.01, 0.1, 1), col = "grey50", col.axis = "grey50", lwd = 0.8)
  if(i %in% seq(1, 4))
    mtext(unique(x$stage_test), side = 3, cex = 0.6, col = "grey50", line = 0.3)
  if(i %in% seq(4, 16, 4))
    mtext(unique(x$stage_train), side = 4, cex = 0.6, col = "grey50", las = 0, line = 0.3)
  text(0.005, 0.7, labels = unique(x$Rsq_sp), col = "grey50")
  if(i == 1) {
    par(xpd = NA)
    legend(0.0032, 8.5, legend = cols_df$proper_name, bty = "n", cex = 1.1, text.col = "grey50", horiz = TRUE, pch = 21, pt.bg = paste0(cols_df$cols_solid, 40), col = paste0(cols_df$cols_solid, 98))
      par(xpd = FALSE)
  }
    })
mtext("Scaled extinction probability (stage that model was built on)", side = 1, outer = TRUE, col = "grey50", cex = 0.6, line = 2.2)
mtext("Scaled extinction probability (stage that model was tested on)", side = 2, outer = TRUE, col = "grey50", cex = 0.6, line = 2.8, las = 0)
par(xpd = NA)
dev.off()
...
stage_models <- llply(c(stage_order, "all"), function(x)
  rlply(5, fit_stage_models(neog, stage_name = x)))
...
<<check-classification-error, results='hide', fig.height=7, fig.width=14, fig.cap='TODO ROC curves'>>=
# get predictions for overall model:
predictions_self <- laply(m, function(x)
  gbm::predict.gbm(x, type = "response", n.trees = NTREES))
neog2 <- neog
neog2$pred <- apply(predictions_self, 2, mean)
...
for(i in 1:length(stage_order)) {
  ...
  w <- paleorisk::get_weights(this_dat$Ex)
  weights <- ifelse(this_dat$Ex == 1, w$ex_weight, w$sur_weight)
  m <- rlply(5, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
      max.lat + lat.range + mean.lat + great.circle + group,
    data = this_dat, n.trees = NTREES,
    interaction.depth = INT.DEPTH, distribution = "bernoulli",
    shrinkage = SHRINKAGE, weights = weights))
  predictions_self <- laply(m, function(x)
    gbm::predict.gbm(x, type = "response", n.trees = NTREES))
  this_dat$pred <- apply(predictions_self, 2, mean)
  out[[i]] <- this_dat
}
stage_model_predictions <- do.call("rbind", out)
neog2$stage <- NULL
neog2$stage <- as.factor("Entire Neogene")
neog2 <- rbind(neog2, stage_model_predictions)
neog3 <- neog2
neog3$class <- NULL
neog3$class <- as.factor("All")
neog2 <- rbind(neog2, neog3)
...
roc_vals <- ddply(neog2, c("stage", "class"), function(x) {
  if(sum(x$Ex) > 0) get_roc(x$Ex, x$pred)
  })
...
ggplot(roc_vals, aes(spec, sens)) +
  ...
  xlim(1, 0) + scale_colour_manual(values = c(RColorBrewer::brewer.pal(6, "Set2"), "#000000")) +
...
# folds_across_mean_provinces <- round(exp(max(by.prov.all$mean.ext)) /
# exp(min(by.prov.all$mean.ext)), 1)
# lq <- quantile(exp(by.prov.all$mean.ext), probs = 0.25)
# uq <- quantile(exp(by.prov.all$mean.ext), probs = 0.75)
# iqr <- as.numeric(uq - lq)
range_between <- diff(range(exp(by.prov.all$mean.ext)))
folds <- ddply(temp, c("PROV_CODE", "PROVINCE"), summarize,
...
# folds_larger <- round(median(folds$iqr) / iqr, 2)
# round(median(folds$fold) / folds_across_mean_provinces, 1)
...
# labs_show <- rep(99, length(labs))
# labs_show[1] <- labs[1]
# labs_show[3] <- labs[3]
# labs_show[5] <- labs[5]
# labs_show[labs_show == 99] <- NA
 labs_show <- sprintf("%.3f", labs)
# labs_show[labs_show == "NA"] <- ""
#labs_show <- labs
...
<<compare-bounding-box-occupancy, fig.width=9, fig.height=3>>=
...
<<compare-bounding-box-lat, fig.width=9, fig.height=6>>=
ggplot(subset(modern_long, variable %in% c("min.lat", "max.lat", "mean.lat")),
  aes(value, value_raw)) +
  geom_point(alpha = 0.2) + facet_grid(variable~class) +
  geom_abline(intercept = 0, slope = 1, lty = 3) +
  coord_fixed(ratio = 1) +
  xlab("Interpolated value") + ylab("Raw value")
@

<<compare-bounding-box-gcd, fig.width=9, fig.height=3>>=
ggplot(subset(modern_long, variable %in% c("great.circle")),
  ...
  coord_fixed(ratio = 1) +
  xlab("Interpolated value") + ylab("Raw value")
# temp <- subset(modern_long, variable %in% "great.circle")
# print(subset(temp, value_raw > value))
@
...
<<alternative-gbm-taxonomy-only-prep, fig.width=7, fig.height=4, out.width="6.25in", results='hide', eval=FALSE>>=
# build calibration models:
val_tax <- rdply(40, validate_gbm(neog,
  model = "Ex ~ group", shrinkage = SHRINKAGE,
  interaction.depth = INT.DEPTH, n.trees = NTREES,
  use_weights = TRUE)$bin_validation,
  .progress = "text")
saveRDS(val_tax, file = "../data/val_tax.rds")
val_tax_summarized <- ddply(val_tax, c("class", "gbm_pred_binned"),
  summarize_val_test)

# build models:
w <- paleorisk::get_weights(neog$Ex)
weights <- ifelse(neog$Ex == 1, w$ex_weight, w$sur_weight)
m_tax <- rlply(5, gbm(Ex ~ group,
  data = neog, n.trees = NTREES, interaction.depth = INT.DEPTH,
  distribution = "bernoulli", shrinkage = SHRINKAGE, weights = weights))

# predict from the models:
predictions_tax <- laply(m_tax, function(x)
  gbm::predict.gbm(x, newdata = modern, type = "response",
    n.trees = NTREES))
modern_tax <- modern
modern_tax$pred_tax <- apply(predictions_tax, 2, mean)

# calibrate:
modern_tax$gbm_pred_binned <- assign_bins(modern_tax$pred_tax)
modern_tax <- plyr::join(modern_tax,
  val_tax_summarized[,c("class", "gbm_pred_binned", "mean_observed")],
  by = c("class", "gbm_pred_binned"))

# merge in spalding map data:
load("../data/interpolated_provs_all_taxa.rda")
d.eco.filled.tax <- interpolated_provs_all_taxa

# merge occurrence data with risk predictions:
d.eco.filled.tax <- merge(d.eco.filled.tax, modern_tax, by = "genus", all.x = FALSE)

# calculate province means:
by.prov.classes.tax <- dplyr::summarise(dplyr::group_by(na.omit(d.eco.filled.tax), class,
    PROV_CODE), mean.ext = mean(log(mean_observed)), N.gen = length(unique(genus)))
by.prov.all.tax <- dplyr::summarise(dplyr::group_by(na.omit(d.eco.filled.tax), PROV_CODE),
  mean.ext = mean(log(mean_observed)), N.gen = length(unique(genus)))

# fake columns:
by.prov.all.tax$Halpern <- 1
by.prov.all.tax$Burrows <- 1

# prepare data for plotting:
er <- maptools::readShapePoly("../data/MEOW2/meow_ecos.shp")
er@data$id <- rownames(er@data)
er_points <- ggplot2::fortify(er, region = "id")
er_dat_tax_class <- plyr::join(er_points, er@data, by = "id")
er_dat_tax_class <- plyr::join(er_dat_tax_class, by.prov.classes.tax, by = "PROV_CODE")
er_dat_tax <- plyr::join(er_points, er@data, by = "id")
er_dat_tax <- plyr::join(er_dat_tax, by.prov.all.tax, by = "PROV_CODE")
@


<<alternative-gbms-plots, results='hide', eval=FALSE>>=
pdf("../figs/class-risk-maps-mean-log-ext-taxonomy-only.pdf", width = 6.45, height = 5.1)
map_class_ext(er_dat_tax_class, plot_column = "mean.ext",
  min_prov_genera = 10, ylab = "Intrinsic extinction probability (over ~23 Myr)",
  picture_files = picture_files,
  silhouette_coords = silhouette_coords,
  yticks = c(0.005, 0.01, 0.02, 0.05, 0.10, 0.20),
  col_range = TRUE, exact_limits = c(0.002, 0.6))
dev.off()

pdf("../figs/hotspots-taxonomy-only.pdf", width = 7, height = 3.5)
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
labs <- c(0.012, 0.014, 0.016, 0.018)
labs_show <- sprintf("%.3f", labs)
map_hotspots(er_dat_tax, min_prov_genera = 120, add_legend = TRUE,
  hotspots = FALSE, at = log(labs), at.labels = labs_show)
par(xpd = NA)
mtext("Intrinsic extinction probability", side = 4, outer = TRUE,
  line = -1.4, las = 0, col = "grey30", cex = 0.9)
dev.off()
...
Because the distribution apparent geographic ranges may be influenced by sampling intensity, it is important to quantidy variability in sampling intensity among provicences.  The number of OBIS records from a given province can serve as a rough index of sampling intensity (Fig.~\ref{fig:plot-n-genera-prov}). 
...
<<obis-residuals-maps, results='hide'>>=
...
map_class_ext(er_dat_class3, yticks = exp(c(-0.5, 0, 0.5)),
...
resids_overall <- ddply(resids, "PROVINCE", summarize, mean(residual.ext))
...
par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0),
  las = 1, mgp = c(1, 0.5, 0), tck = -0.2)
# labs <- seq(0.008, 0.016, 0.002)
# labs_show <- rep(99, length(labs))
# labs_show[1] <- labs[1]
# labs_show[3] <- labs[3]
# labs_show[5] <- labs[5]
# labs_show[labs_show == 99] <- NA
# labs_show <- sprintf("%.3f", labs_show)
# labs_show[labs_show == "NA"] <- ""
#labs.ns <- c(0.006, 0.008, 0.012)
...
<<plot-richness-genera-prov, fig.cap="Spatial distribution of \\textbf{mean species richness} in the contemporary dataset.", fig.width=6.25, fig.height=4.8, out.width="6in">>=
...
% TODO fix figure number references (main text ones) in the SOM
% throughout
