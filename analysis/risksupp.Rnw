...
val_test2 <- rdply(100, fit_models(stcen2, shrinkage = 0.1,
...
<<predict-risk-modern>>=
stcen3 <- readRDS("../data/standardized-predictors-cenozoic-obis.rds")
stcen2 <- gdata::drop.levels(subset(stcen3,stcen3$stage_top < 22 &
    stcen3$stage_top != 0))
m <- gbm(Ex ~ richness + occupancy + occurrences + min.lat + max.lat +
  lat.range + mean.lat + great.circle + class + group, data =
  smote(stcen2), n.trees = 300, interaction.depth = 1,
  distribution = "bernoulli", shrinkage = 0.1)
temp_dat <- readRDS("../data/standardized-predictors-cenozoic-obis.rds")
#modern <- subset(temp_dat, !class %in% c("Testudines", "Malacostraca") &
#  stage == "Modern")
modern <- subset(temp_dat, stage == "Modern")
