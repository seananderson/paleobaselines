...
install.packages("~/Documents/riskmaps/paleorisk/", repos = NULL, type = "source")
@
...
Table.S1 <- ddply(dat,.(Interval,class),each(Genera ,Occurrences))
Table.S1 <- Table.S1[(Table.S1$Interval != "Modern_merged" & Table.S1$class != "Malacostraca" & Table.S1$class != "Foraminifera" & Table.S1$class != "Testudines"),]
Table.S1$Interval <- revalue(Table.S1$Interval, c("Modern_raw" = "Modern"))
Table.S1$Interval <- factor(Table.S1$Interval, levels=c("Lower Miocene","Middle Miocene","Upper Miocene","Plio-Pleistocene","Modern"))
Table.S1 <- Table.S1[order(Table.S1$Interval, Table.S1$class),]
write.csv(Table.S1,"../figs/table.S1.data.csv")
@
...
We will now standardize the predictor data. This means we will adjust the range of predictor variables to be between X and X in the case of X...
...
TODO: Add comment about colinearity
...
<<drop-malacostraca>>=
stcen2 <- drop.levels(stcen)
stcen2$class <- as.character(stcen2$class)
stcen2$class[stcen2$class == "Malacostraca"] <- "Decapoda"
stcen2$class <- as.factor(stcen2$class)
...
modern <- subset(temp_dat,
  !class %in% "Testudines" & stage == "Modern_merged")
modern$class <- as.character(modern$class)
modern$class[modern$class == "Malacostraca"] <- "Decapoda"
...
# make the figure:
pdf("../figs/class-risk-maps-mean-log-ext.pdf", width = 6.25, height = 6.0)
...
\subsection{Exploring contemporary sampling intensity} We will plot the number of genera per province by class:
