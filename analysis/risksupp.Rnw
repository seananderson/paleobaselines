...
Rsqs$X <- rep(.001,length(Rsqs[,1]))
Rsqs$Y <- rep(.70,length(Rsqs[,1]))
...
p <- p + geom_point(data=cross_pred,
  ...
  xlab("Extinction probability (model built on same stage)") +
  ylab("Extinction probability (model built on different stage)") +
