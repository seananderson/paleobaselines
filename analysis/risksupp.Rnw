...
<<partial-plots-base, child='make-partial-dependence-baseplot.Rnw', echo=FALSE, results=FALSE>>=
@
...
by.prov.classes <- summarise(group_by(na.omit(d.eco.filled), class,
    PROV_CODE), mean.ext = mean(log(mean_observed + 0.001)), median.ext =
  ...
  mean(min.lat), mean.max.lat = mean(max.lat), mean.mean.lat = mean(mean.lat),
  mean.gcd = mean(great.circle), mean.lat.range = mean(lat.range),
  mean.richness = mean(richness))

by.prov.all <- summarise(group_by(na.omit(d.eco.filled), PROV_CODE), mean.ext =
  ...
  mean(max.lat), mean.mean.lat = mean(mean.lat), mean.gcd =
  mean(great.circle), mean.lat.range = mean(lat.range), mean.richness =
  mean(richness))
...
tropical_test <- plyr::join(tropical_test, by.prov.classes[, c("PROV_CODE",
    "class", "Lat_Zone", "mean.ext")], by = c("PROV_CODE", "class", "Lat_Zone"))
