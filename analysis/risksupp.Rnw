...
This document is written with the \pkg{knitr} package for the \proglang{R} statistical environment. It can be re-created by running \fn{knitr::knit2pdf()} on the \texttt{.Rnw} file. A number of functions that are used in this analysis have been compiled into an \proglang{R} package \pkg{paleorisk}.
...
Then we'll load other packages that are needed for our analysis:
...
We'll take a look at calibrated predictions vs.\ the raw model predictions (with weighted observations):
...
<<test-tropical-drivers, fig.width=18, fig.height=11.5, out.width="7in">>=
