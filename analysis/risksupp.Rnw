...
xtable::print.xtable(xtable::xtable(Table.S1, digits = c(0, 0, 0, 0, 0),
    caption = "Total number of unique genera and occurrences of those genera in the Paleobiology Database (Neogene intervals) and OBIS database (modern) for major taxonomic groups and time intervals used in our analysis."),
...
Taxonomic group & Ordinal to family rank taxonomic assignments for each genus\\
\bottomrule
\end{tabular}
\end{table}
