...
<<get-oos-rsquared, fig.width=7.5, fig.height=7, echo=FALSE>>=
oos_rsq_glm <- plyr::ddply(val_test, "class", function(x) {
  require(mgcv)
 ...
 print(oos_rsq_glm)
 print(range(oos_rsq_glm$V1))
 print(median(oos_rsq_glm$V1))

glm_pred <- plyr::ddply(val_test, "class", function(x) {
...
<<plot-dev-explained, fig.width=7.5, fig.height=7, fig.cap="TODO fix caption. This is for percent deviance explained in 2-fold cross-validation. Add those values to panels? Shown is a GLM (logistic regression) fit.">>=
ggplot(val_test_summarized,
  aes(gbm_pred_binned, mean_observed)) +
  geom_point(data = val_test, aes(gbm_pred_binned, obs_ext_prob),
    alpha = 0.1) +
  ...
  labs(size = "Median sample #") +
  geom_line(data = glm_pred, aes(gbm_pred_binned, pred), col = "red", lwd = 1)
@
...
<<alternative-gbms, eval=FALSE>>=
val_test_base <- rdply(15, validate_gbm(neog, shrinkage = 0.05,
    interaction.depth = 1, n.trees = NTREES,
    use_weights = TRUE)$bin_validation)
val_test_shr <- rdply(15, validate_gbm(neog, shrinkage = 0.01,
    interaction.depth = 1, n.trees = NTREES,
    use_weights = TRUE)$bin_validation)
val_test_int <- rdply(15, validate_gbm(neog, shrinkage = 0.05,
    interaction.depth = 4, n.trees = NTREES,
    use_weights = TRUE)$bin_validation)
val_test_ntrees <- rdply(15, validate_gbm(neog, shrinkage = 0.05,
    interaction.depth = 1, n.trees = 10000,
    use_weights = TRUE)$bin_validation)
@

<<summarize-alt-gbms, eval=FALSE>>=
...
<<plot-within-stage-tests, fig.width=8.5, fig.height=7, out.width="6.25in", fig.cap="TODO fix caption. Within interval 2-fold cross-validation prediction. Fit shown is a logistic regression. (probability with weights for numbers of samples).">>=
ggplot(data = within_stage, 
  aes(x = gbm_pred_binned, y = obs_ext_prob, size = sample_n, colour = class)) +
  ...
  geom_line(data = glm_pred, aes(gbm_pred_binned, pred), col = "darkgrey", lwd = 1)
@
...
Rsqs <- ddply(cross_pred, plyr::.(stage_train,stage_test), plyr::each(Rsq))
...
p <- ggplot()
p <- p + geom_point(data=cross_pred,
    aes(pred_self, pred, colour = class), alpha=0.3)  +
...
The median Spearman's rank correlation for the cross-interval predictions is \Sexpr{median(Rsq$Rsq)} and the rank correlations range from \Sexpr{min(Rsq$Rsq)} to \Sexpr{max(Rsq$Rsq)}.
...
<<plot-class-prov-ext-raw-ranges">>=
...
<<remove-single-province-genera-class-map>>=
...
<<remove-single-province-genera-overall-map>>=
