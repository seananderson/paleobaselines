...
<<plot-genus-predictions-against-gcd2, fig.cap='Predicted intrinsic risk versus great circle distance for modern genera in each of six classes.  The blue points represent individual genera, the black contour lines indicate the density of genera, and the red line represents the mean intrinsic risk of all genera.', fig.width=9, fig.height=5.5>>=
d2 <- ddply(modern, "class", transform, 
group_id = as.numeric(as.factor(as.character(as.numeric(group)))))

ggplot(d2, aes(y = jitter(gbm_pred_binned, amount = 0.02), x = jitter(great.circle, amount = 800))) +  geom_hline(yintercept=mean(d2$gbm_pred_binned),colour="red") + geom_point(alpha=.5,pch=1,size=2,colour="cornflowerblue") + facet_wrap(~class) + theme_bw() + xlab("Great circle distance") + ylab("Predicted intrinsic risk by genus") + theme(strip.text.x = element_text(size = 8), axis.text = element_text(size = 8)) + theme(legend.position = "none") + stat_density2d(colour="black",size=.25,bins=15)
@
