...
n_genera_singleton_1.0 <- nrow(filter(neog_temp, singleton == 0 & prop_comp == 1.0))
...
# preservation prob. == 1 *and* no singleton (stage) genera:
if(!file.exists("../data/val_test_singleton.rds")) {
  val_test_singleton <- ldply(1:100, function(i)
    validate_gbm(neog[neog$singleton == 0 & neog$prop_comp == 1, ],
      shrinkage = SHRINKAGE,
      interaction.depth = INT.DEPTH, n.trees = NTREES,
      use_weights = TRUE)$bin_validation, .parallel = TRUE)
...
# combine the preservation probability == 1.0 with removing singletons:
neog_subset <- neog[neog$singleton == 0 & neog$prop_comp == 1, ]
w <- paleorisk::get_weights(neog_subset$Ex)
weights <- ifelse(neog_subset$Ex == 1, w$ex_weight, w$sur_weight)
