...
% 
% Plio.Modern$Interval <- factor(Plio.Modern$Interval)
...
m <- rlply(4, gbm(Ex ~ richness + occupancy + occurrences + min.lat +
    max.lat + lat.range + mean.lat + great.circle + group,
  data = neog, n.trees = NTREES, interaction.depth = INT.DEPTH,
  distribution = "bernoulli", shrinkage = SHRINKAGE, weights = weights))
@
...
<<min-gen-thresh-sensitivity, fig.height=8.5, fig.width=13, fig.cap='TODO Cutoff effect.'>>=
par(mfrow = c(2, 2))
