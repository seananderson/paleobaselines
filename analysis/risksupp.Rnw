...
As described above, we used GBM models built on the Neogene fossil record to produce intrinsic risk extimates for modern genera.  In order to map out the distribution of mean intrinsic risk across coastal biogeographic provinces, we summed intrinsic risk estimates for all genera occupying a given province (following the minimum bounding box interpolation) and divided by the total number of genera.  This was done both separately for each major taxonomic group (Fig. 2) and for all genera combined (Fig. 3).
