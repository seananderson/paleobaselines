...
extract_ranges <- function() {
  ...
  ranges <- function(df){
    if(length(na.omit(df$latitude))>1){
      ...
      richness <- mean(unique(droplevels(df$richness)))
  ...
  download.file("https://dl.dropboxusercontent.com/u/254940/riskmaps-data/4.6.2014.pbdb.occs.rda",
    "../data/4.6.2014.pbdb.occs.rda",
    method = "curl")
  load("../data/4.6.2014.pbdb.occs.rda")
  
  # Estimation of proportional completness using FR2 bins
  # From Carl on 20140923
  int <- length(which(levels(pbdb$FR2_bin)!=""))
  times <- function(x) {
    y <- unique(x$FR2_bin)
    z <- length(which(y != ""))
    data.frame(prop_comp = z/int)
  }
  ...
  FR2_bin <- c("Pleistocene","Pliocene","Plio-Pleistocene","Upper Miocene","Middle Miocene","Lower Miocene","Chattian","Rupelian")
  stage.name <- c("Plio-Pleistocene","Plio-Pleistocene","Plio-Pleistocene","Upper Miocene","Middle Miocene","Lower Miocene","Chattian","Rupelian")
  ...
  gen.LAs <- ddply(pbdb,.(clgen),each(gen.LA,gen.FA),.progress="text")
  ...
  #PlioPleis  <- droplevels(subset(data,data$Interval=="Plio-Pleistocene"))
  #Modern <- droplevels(subset(data,data$Interval=="Modern_raw"))
