...
extract_ranges <- function() {
  ...
  # Delete "../data/OBIS.ranges.processed.csv to make this re-run
  if(!file.exists("../data/OBIS.ranges.processed.csv")) {
    ...
    byGenus.OBIS <- read.csv("../data/OBIS.ranges.processed.csv")
  }
  ...
  bottom <- c(5.333,5.33,5.33,11.608,15.97,23.03)
  ...
  pbdb.gen.cats <- read.csv("../data/PBDB.genera.Groups.2.0.csv", header=TRUE)
...
pbdb <- merge(pbdb,pbdb.fams,by.x="family_name",by.y="family",all.x=TRUE)
  ...
  # delete ../data/PBDB.ranges.process.csv to regenerate:
  if(!file.exists("../data/PBDB.ranges.processed.csv")) {
  ...
  write.table(byGenus.PBDB,"../data/PBDB.ranges.processed.csv", sep = ",")
    ...
    byGenus.PBDB <- read.csv("../data/PBDB.ranges.processed.csv")
  }
