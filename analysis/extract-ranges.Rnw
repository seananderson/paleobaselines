...
extract_ranges <- function() {
  ...
  #pbdb2 <- readRDS('~/Dropbox/nescent_extinction_map/data/PaleoDB downloads/6.25.2014/6.25.2014.occs.rds')
  #  load('~/Dropbox/nescent_extinction_map/data/PaleoDB downloads/4.6.2014/4.6.2014.occs.rda')

  # Warning, sometimes this crashes my R session:
  #  download.file("https://dl.dropboxusercontent.com/u/254940/riskmaps-data/4.6.2014.pbdb.occs.rda",
  #    "../data/4.6.2014.pbdb.occs.rda", method = "curl")
  # If that's the case, you'll need to download this file and place it in the
  # ../data/ folder
  ...
  lookup_pbdb  <- data.frame(FR2_bin =
    c("Upper Miocene", "Middle Miocene", "Lower Miocene",
      "Pliocene", "Pleistocene"),
    FR2_num = c(1:5), stringsAsFactors = FALSE)

  pbdb <- plyr::join(pbdb, lookup_pbdb)

  pres_prob <- function(x) {
    out <- length(unique(x$FR2_num)) /
      (max(x$FR2_num) - min(x$FR2_num) + 1)
     data.frame(prop_comp = out)
  }
#   d <- data.frame(FR2_num = c(1, 2, 3, 5))
#   pres_prob(d)
#
#   d <- data.frame(FR2_num = c(1, 2, 5))
#   pres_prob(d)
#
#   d <- data.frame(FR2_num = c(1, 2, 3, 4, 5))
#   pres_prob(d)
#
#   d <- data.frame(FR2_num = c(2, 3, 4, 5))
#   pres_prob(d)
#
#   d <- data.frame(FR2_num = c(2, 4, 5))
#   pres_prob(d)

  # Estimation of proportional completeness using FR2 bins
  # From Carl on 20140923
#  int <- length(which(levels(pbdb$FR2_bin)!=""))
#  times <- function(x) {
#    y <- unique(x$FR2_bin)
#    z <- length(which(y != ""))
#    data.frame(prop_comp_carl = z/int)
#  }
#
#   d <- data.frame(FR2_num = c(1, 2, 3, 4, 5))
#   pres_prob(d)
 ...
 # prop_comp <- plyr::ddply(pbdb, "occurrence.genus_name", times)
 # pbdb <- plyr::join(pbdb, prop_comp, by = "occurrence.genus_name")
  ...
  FR2_bin <- c("Pleistocene", "Pliocene", "Plio-Pleistocene", "Upper Miocene",
    "Middle Miocene", "Lower Miocene", "Chattian", "Rupelian")
  stage.name <- c("Plio-Pleistocene", "Plio-Pleistocene", "Plio-Pleistocene",
    "Upper Miocene", "Middle Miocene", "Lower Miocene", "Chattian", "Rupelian")
...
##  pbdb <- pbdb[!(is.na(pbdb$taxon_no) | pbdb$taxon_no==0), ] # not removing anything
  ...
  gen.LAs <- plyr::ddply(pbdb, plyr::.(clgen), plyr::each(gen.LA,gen.FA), .progress = "text")
  ...
  pbdb$Singleton <- ifelse(pbdb$gen.LA == pbdb$gen.FA & pbdb$genus_extant != "yes",1,0)
  ...
  byGenus.PBDB <- plyr::ddply(pbdb, plyr::.(stage.name, top, class,
    Group, occurrence.genus_name), ranges, .progress="text")
  ...
  data$single_occurrence <- 0
  data[data$genus %in% single_occurrences, "single_occurrence"] <- 1
