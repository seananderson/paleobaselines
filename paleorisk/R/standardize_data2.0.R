...
#data <- gdata::drop.levels(subset(df, df$use==1 & df$occurrences >=
#min_pbdb_occurrences & df$OBIS_occurrences >= min_modern_occurrences &
#df$Num_Stage >= minimum_duration & df$class != "Foraminifera" &
#(df$OBIS_Ranges == input_ranges | df$OBIS_Ranges == 0)))
#df <- dat
#df <- gdata::drop.levels(subset(df, df$Interval_Name == "Plio-Pleistocene"))

data  <- gdata::drop.levels(subset(df, df$occurrences >= min_occurrences
                                   & df$class != "Foraminifera"))
...
stand_dat <- gdata::drop.levels(na.omit(data.frame(stage, stage_top, class, group,genus, richness, occupancy, occurrences, min.lat, max.lat, lat.range,mean.lat, great.circle, tropical_only, Ex)))
